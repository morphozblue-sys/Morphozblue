...
$s_no=$_GET['id'];
$query="select * from annualfee_certificate where s_no='$s_no'";
$run=mysqli_query($conn73,$query) or die(mysql_error());
while($row=mysqli_fetch_assoc($run)){
$annualfee_student_name = $row['annualfee_student_name'];
$annualfee_student_father_name = $row['annualfee_student_father_name'];
$annualfee_school_name = $row['annualfee_school_name'];
$annualfee_current_year_from = $row['annualfee_current_year_from'];
$annualfee_current_year_to = $row['annualfee_current_year_to'];
$annualfee_type = $row['annualfee_type'];
$annualfee_issue_date = $row['annualfee_issue_date'];
$annualfee_student_roll_no = $row['annualfee_student_roll_no'];

}
$query1="select * from student_admission_info where student_roll_no='$annualfee_student_roll_no' and session_value='$session1'";
...
global $school_info_school_address,$annualfee_student_name,$student_date_of_admission,$student_class,$student_admission_number,$student_mother_name,$school_info_school_contact_no,$school_info_school_website,$school_info_school_email_id,$annualfee_student_father_name,$bonafied_school_name,$annualfee_current_year_from,$annualfee_current_year_to,$annualfee_type,$annualfee_issue_date,$annualfee_school_name,$annualfee_student_roll_no,$school_info_school_name,$school_info_logo,$path1;
   
    $this->Image('../certificate_image/annual.jpg',1,1,207,295);
    // $this->Image('../certificate_image/annual-croped.jpg',164,250,20,6);
    ...
    $this->SetFont('Times','B',12);
	$this->SetTextColor(135,109,52);
    //$this->Cell(106,9,'Website :'.$school_info_school_website,0,0,'');
    ...
    $this->Cell(89,6,$annualfee_student_name,0,0,'C');
    $this->Ln(); 
	
	$this->SetXY(35,122);
	$this->SetFont('Times','B',11);
	$this->SetTextColor(76,0,153);
    $this->Cell(65,6,$annualfee_student_father_name,0,0,'C');
    $this->Ln(); 
    ...
    $this->SetFont('Times','B',12);
	$this->SetTextColor(76,0,153);
    $this->Cell(120,6,$annualfee_school_name,0,0,'C');
    $this->Ln(); 	
    ...
    $this->SetFont('Times','B',12);
	$this->SetTextColor(76,0,153);
    $this->Cell(60,6,$annualfee_current_year_from.'-'.$annualfee_current_year_to,0,0,'C');
    $this->Ln();

    $this->SetXY(60,158);
    $this->SetFont('Times','B',12);
	$this->SetTextColor(76,0,153);
    $this->Cell(35,6,$annualfee_type,0,0,'C');
    $this->Ln();
...
$file_name="Annualfee_certificate_".$annualfee_student_name.".pdf";
