...
global $conn73,$school_info_school_name,$school_info_school_contact_no,$session1,$school_info_logo,$student_name,$student_class,$student_roll_no,$student_date_of_birth,$student_father_contact_number,$student_photo,$student_image,$roll_no,$school_roll_no,$school_info_school_code,$student_gender,$school_info_school_address,$path1;
...
$que1="select * from student_admission_info where student_roll_no='$roll_no[$j]' and session_value='$session1'";
    ...
    while($row1=mysqli_fetch_assoc($run1)){
	 ...
	 $student_guardian_photo=$row1['student_guardian_image_name'];
	 $student_photo1=$_SESSION['amazon_file_path']."student_documents/".$student_photo;$student_image=str_replace(" ","%20",$student_image);
	 $student_guardian_photo1=$_SESSION['amazon_file_path']."student_documents/".$student_guardian_photo;
	 $student_guardian_photo1=str_replace(" ","%20",$student_guardian_photo1);
	// $student_guardian_photo1="data:image/jpeg;base64,".$student_guardian_photo;
}
if($horizontal>2 && $vertical>1){
  ...
  $width=$width1*$horizontal;
	...
	$this->Image('../id_card_image/id_card_background_header_1.png',10+$width,9+$height,80,13.5);
		...
		if($student_guardian_photo==null){
		$this->Image('../../../images/blank.jpg',75+$width,23+$height,14.6,14.6);
		}else{
		$this->Image($student_guardian_photo1,75+$width,23+$height,14.6,14.6,'jpeg');
		}
...
if($horizontal>0)
  ...
  $this->SetLeftMargin(10+$width);  
		...
		$this->Cell(50,15,'Guardian ID Card',1,0,'C');
		$this->Cell(15,15,'',1,0,'C');
		$this->Ln();
		//--------------------------------------------
		$this->Cell(52,0,'',0,0,'C');
		...
		$this->SetTextColor(3,3,3);
		$this->Cell(40,4,'Fn/Mn :',0,0,'L');
		$this->Cell(40,4,''.$student_father_name."/".$student_mother_name,0,0,'R');
		$this->Ln();
		//--------------------------------------------
		$this->SetFont('Times','B',9);
		$this->SetTextColor(3,3,3);
		$this->Cell(40,4,'Carrying Prsn',1,0,'L');
		$this->Cell(40,4,''.$student_guardian_name,1,0,'R');
		$this->Ln();
		//--------------------------------------------
