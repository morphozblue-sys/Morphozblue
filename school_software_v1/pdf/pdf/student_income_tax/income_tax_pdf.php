...
$student_roll_no=$_POST['student_data'];
$total_student=0;
...
function Table1()
{
global  $conn73,$conn73,$fees_category,$session1,$filter37,$student_admission_number,$student_class,$student_roll_no,$student_class_section,$student_district,$student_adress,$student_mother_name,$student_father_name,$student_name,$school_info_school_address,$student_photo,$student_image,$total_student,$school_info_school_name,$school_info_school_district,$school_info_logo,$path1;
...
$this->AddPage();
	...
	$this->SettextColor(0,0,0);
	$this->SetFont('Times','B',11);
	$this->Cell(79,10,''.strtoupper($student_mother_name[$f]),0,0,'');
	$this->SetFont('Times','',12);
	$this->Cell(23,10,'Resident of',0,0,'');
	$this->SetFont('Times','B',11);
	$this->Cell(70,10,''.strtoupper($student_adress[$f]),0,0,'');
	$this->SetFont('Times','',12);
	...
	$this->Ln();
	$whw="select grand_total from common_fees_student_fee   where  student_roll_no='$student_roll_no[$f]' and session_value='$session1' and fee_status='Active'";
	$sql6=mysqli_query($conn73,$whw);
	while($res6=mysqli_fetch_assoc($sql6)){
	  
	      $grand_total=$res6['grand_total'];
