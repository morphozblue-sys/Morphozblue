...
function Table1()
{
	...
	$run000=mysqli_query($conn73,$query000);
...
while($row121=mysqli_fetch_assoc($run121)){
	...
	$editable_reciept_no=$row1['editable_receipt_no'];
	...
	$this->SetTextColor(0,0,0);
    $this->Cell(58,2,"Dist.  -  ".$school_info_school_city.$school_info_school_district,0,0,'C');
    $this->Ln();
...
//	}
	...
	$this->Cell(35,5,"RECIEPT NO. : ".$editable_reciept_no,0,0,'R');
	}else{
	...
	$serial_no_trnsprt=$serial_no_pnlty;
	...
	$serial_no_prev=$serial_no_trnsprt;
	if($student_previous_year_fee_paid_total!='' && $student_previous_year_fee_paid_total>0){
	$this->SetLeftMargin(10);
    $this->SetFont('helvetica','B',10);
	$this->SetTextColor(0,0,0);
	if($student_previous_year_fee_paid_total!='' && $student_previous_year_fee_paid_total>0){
	$serial_no_pre=$serial_no_prev+1;
	$serial_no_pre_fee=$serial_no_pre.'.';
	$pre_fee_name='Previous Dues Amount';
	$pre_fee_amount=$student_previous_year_fee_paid_total;
	}else{
	$serial_no_pre_fee='';
	$pre_fee_name='';
	$student_transport_fee_paid_total11='';
    ...
    $this->Cell(-10,5,"",0);
	$this->Cell(5,6,"".$serial_no_pre_fee,0);
	$this->Cell(48,6,"".$pre_fee_name,0);
	$this->Cell(18,6,''.$pre_fee_amount,0,0,'C');
	$this->Ln();	
	...
	if($blank_field_2!='' && $blank_field_2>0){
	$this->SetLeftMargin(10);
    $this->SetFont('helvetica','B',10);
	$this->SetTextColor(0,0,0);
	if($blank_field_2!='' && $blank_field_2>0){
	$discount_name='Discount'.'('.$blank_field_1.')';
	$discount_amount=$blank_field_2;
	}else{
	$discount_name='';
	$discount_amount='';
	}
    $this->Cell(-10,5,"",0);
	$this->Cell(5,6,"",0);
	$this->Cell(48,6,"".$discount_name,0);
	$this->Cell(18,6,''.$discount_amount,0,0,'C');
	$this->Ln();	
...
$file_name="fee_receipt_thermal.pdf";
