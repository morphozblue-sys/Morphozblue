<?php 
          ...
          //////////////////////////////////         Practical Subject       /////////////////////////////////////////////
           ...
           for($e1=0;$e1<$total_exam;$e1++)
           {
            for($s1=0;$s1<$total_subject;$s1++)
            {
             for($i=0;$i<$subjet_category_count[$s1];$i++)
             {
              //if($subject_marks[$e1][$s1][$i]<$student_minimum_marks[$e1][$s1][$i] || $student_minimum_marks[$e1][$s1][$i]==0)
    ...
    global $subject_marks,$subject_other_marks,$subject_practical_marks,$subject_clear,$subject_other_clear,$subject_practical_clear;
     ...
     $exam_code=$_GET['exam_type'];
		 ...
		 $this->Cell(144,7,"".strtoupper($exam_name_show),1,0,'C');
		 $this->Ln();
         ...
         for($e1=0;$e1<$total_exam;$e1++)
         {
         $this->Cell($exam_adust,8,"Minimum",1,0,'C');
         $this->Cell($exam_adust,8,"Maximum",1,0,'C');
         $this->Cell($exam_adust,8,"Obtain",1,0,'C');
         $this->Cell($exam_adust,8,"Result",1,0,'C');
         }
         $this->Ln();
         ...
         if($total_subject!=0)
         {
             ...
             for($s1=0;$s1<$total_subject;$s1++)
             {
              ...
              $obt_marks[$s1]=0;    
              $this->Cell(-11);     
              $this->Cell(56,$adjust,"".$subject[$s1],1,0,'C');  
              for($e1=0;$e1<$total_exam;$e1++)
              {
              $min_marks[$s1]=array_sum($student_minimum_marks[$e1][$s1]);
              $max_marks[$s1]=array_sum($student_maximum_marks[$e1][$s1]);
              $obt_marks[$s1]=array_sum($subject_marks[$e1][$s1]);      
              $this->Cell($exam_adust,$adjust,"".$min_marks[$s1],1,0,'C');
              $this->Cell($exam_adust,$adjust,"".$max_marks[$s1],1,0,'C');
              $this->Cell($exam_adust,$adjust,"".$obt_marks[$s1],1,0,'C');
              $result=$subject_clear[$e1][$s1]=='Yes'?'CLEAR':'NOT CLEAR';
              $final_result=$result;
              $this->Cell($exam_adust,$adjust,"".$result,1,0,'C');
              }
              $final_minimum=array_sum($min_marks);
              $final_maximum=array_sum($max_marks);
              $final_obtain=array_sum($obt_marks);
              $this->Ln();
             }
             $this->Cell(-11);     
             $this->SetFont('Times','B',14); 
             $this->Cell(56,12,"Total",1,0,'C');  
             for($e1=0;$e1<$total_exam;$e1++)
             {
             $this->Cell($exam_adust,12,"".$final_minimum,1,0,'C');
             $this->Cell($exam_adust,12,"".$final_maximum,1,0,'C');
             $this->Cell($exam_adust,12,"".$final_obtain,1,0,'C');
             $this->Cell($exam_adust,12,"".$final_result,1,0,'C');
             }
