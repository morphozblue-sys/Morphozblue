<?php 
   ...
   $student_admission_number[$student_roll_no1]=$row2['student_admission_number'];
    ...
    $student_image_name[$student_roll_no1]=str_replace(" ","%20",$student_image[$student_roll_no1]);
    $total_student++;
    }
    ...
    function get_student_marks($roll_no)
    {
       ...
       global $filter37,$exam_type2,$subject,$subject_code,$subjet_category_set,$subjet_category_name,$student_maximum_marks,$student_minimum_marks,$exam_marks_add,$total_subject,$subject_other,$subject_code_other,$subjet_other_category_set,$subjet_other_category_name,$student_other_maximum_marks,$student_other_minimum_marks,$exam_other_marks_add,$subject_practical,$subjet_practical_category_name,$subjet_practical_category_index,$student_practical_maximum_marks,$student_practical_minimum_marks,$exam_practical_marks_add,$subject_clear,$subject_other_clear,$subject_practical_clear,$percent_subwise;
        ...
        while($row2 = mysqli_fetch_assoc($run2)){
          $subject_marks=array();
          $percent_subwise=array();
          $subject_marks_exam_wise_obt=array();
          $subject_marks_exam_wise_max=array();
          $subject_marks_subwise_obt=array();
          $subject_marks_subwise_max=array();
          $clear_nc_subjectwise=array();
          ...
          //////////////////////////////////         Practical Subject       /////////////////////////////////////////////
           ...
           for($e1=0;$e1<$total_exam;$e1++)
           {
            for($s1=0;$s1<$total_subject;$s1++)
            {
             for($i=0;$i<$subjet_category_count[$s1];$i++)
             {
             $subject_marks_exam_wise_obt[$e1][$s1]+=$subject_marks[$e1][$s1][$i];
             $subject_marks_exam_wise_max[$e1][$s1]+=$student_maximum_marks[$e1][$s1][$i];
             
             $subject_marks_subwise_obt[$s1]+=$subject_marks[$e1][$s1][$i];
             $subject_marks_subwise_max[$s1]+=$student_maximum_marks[$e1][$s1][$i];
             }
              $percent=$subject_marks_exam_wise_obt[$e1][$s1]*100/$subject_marks_exam_wise_max[$e1][$s1];
              if($percent>=40)
           ...
           for($s1=0;$s1<$total_subject;$s1++)
           {
            $percent_subwise[$s1]=$subject_marks_subwise_obt[$s1]*100/$subject_marks_subwise_max[$s1];
           }
           ////////////////////////////////////////////////////////
           for($e1=0;$e1<$total_exam;$e1++)
           {
    ...
    {
        global $school_info_school_name,$school_info_school_address,$school_info_school_district,$school_info_school_state,$student_name,$student_father_name,$student_class,$student_address,$student_date_of_Birth,$school_roll_no,$school_info_logo,$path1,$student_photo,$student_image_name,$student_admission_number;
        $this->Ln(-3);
		...
		$this->Image($student_image_name[$student_roll],180,5,21,21,'jpeg');
		} 
    ...
    $exam_code="";
		 ...
		 $all_exam_clear_no='CLEAR';
		 for($s1=0;$s1<$total_subject;$s1++)
		 {
		    
		     //echo $percent_subwise[$s1].'<br>';
		  if($percent_subwise[$s1]<40)
		  $all_exam_clear_no='N.C';
		 }
		 $this->Cell(-11);
		 $this->SetFont('Times','B',11.5); 
		 $this->SetTextColor(0,0,0);
		 $this->Cell(36,22,"".strtoupper("Subject Name"),1,0,'C');
		 $this->Cell(164,7,"".strtoupper($exam_name_show),1,0,'C');
		 $this->Ln();
		 
		 $this->Cell(-11);
		 $this->SetFont('Times','B',11.5); 
		 $this->SetTextColor(0,0,0);
		 $this->Cell(36,21,"",0,0,'C');
		 for($e1=0;$e1<$total_exam;$e1++)
         {
         $this->SetFont('Times','B',7);      
		 $this->Cell(164/$total_exam,7,"".strtoupper($exam_type2[$e1]),1,0,'C');
         }
		 $this->Ln();
         ...
         for($e1=0;$e1<$total_exam;$e1++)
         {
         $this->Cell($exam_adust,8,"Min",1,0,'C');
         $this->Cell($exam_adust,8,"Max",1,0,'C');
         $this->Cell($exam_adust,8,"Obt",1,0,'C');
         $this->Cell(14,8,"Result",1,0,'C');
         $examwise_result[$e1]="CLEAR";
         $examwise_total_max[$e1]=0;
         $examwise_total_min[$e1]=0;
         $examwise_total_obt[$e1]=0;
         }
         $this->Ln();
         ...
         for($e1=0;$e1<$total_exam;$e1++)
         {
          for($s1=0;$s1<$total_subject;$s1++)
          {
           $min_marks[$e1][$s1]=array_sum($student_maximum_marks[$e1][$s1])*40/100;      
          }
         }
         ...
         if($total_subject!=0)
         {
             ...
             for($s1=0;$s1<$total_subject;$s1++)
             {
              ...
              $this->Cell(36,$adjust,"".$subject[$s1],1,0,'C');  
              for($e1=0;$e1<$total_exam;$e1++)
              {
              $this->SetFont('Times','B',11);      
              //$min_marks[$e1][$s1]=array_sum($student_minimum_marks[$e1][$s1]);
              $max_marks[$e1][$s1]=array_sum($student_maximum_marks[$e1][$s1]);
              $obt_marks[$e1][$s1]=array_sum($subject_marks[$e1][$s1]);      
              $this->Cell($exam_adust,$adjust,"".$min_marks[$e1][$s1],1,0,'C');
              $this->Cell($exam_adust,$adjust,"".$max_marks[$e1][$s1],1,0,'C');
              $this->Cell($exam_adust,$adjust,"".$obt_marks[$e1][$s1],1,0,'C');
              if($subject_clear[$e1][$s1]=='No')
              {
              $examwise_result[$e1]="N.C";
              $final_result="NOT CLEAR";
              }
              $this->SetFont('Times','B',8);
              if($subject_clear[$e1][$s1]=='Yes')
              $result='CLEAR';
              else
              $result='N.C';
              $this->Cell(14,$adjust,"".$result,1,0,'C');
              
              $examwise_total_max[$e1]+=$max_marks[$e1][$s1];
              $examwise_total_obt[$e1]+=$obt_marks[$e1][$s1];
              $examwise_total_min[$e1]+=$min_marks[$e1][$s1];
              }
             ...
             $this->Cell(-11);     
             $this->SetFont('Times','B',14); 
             $this->Cell(36,12,"Total",1,0,'C');  
             $this->SetFont('Times','B',12); 
             for($e1=0;$e1<$total_exam;$e1++)
             {
             $this->SetFont('Times','B',11);      
             $this->Cell($exam_adust,12,"".$examwise_total_min[$e1],1,0,'C');
             $this->Cell($exam_adust,12,"".$examwise_total_max[$e1],1,0,'C');
             $this->Cell($exam_adust,12,"".$examwise_total_obt[$e1],1,0,'C');
             $this->SetFont('Times','B',8);
             $this->Cell(14,12,"".$examwise_result[$e1],1,0,'C');
             $final_minimum+=$examwise_total_min[$e1];
             $final_maximum+=$examwise_total_max[$e1];
             $final_obtain+=$examwise_total_obt[$e1];
             }
