...
global $conn73,$school_info_school_name,$ext,$school_info_school_contact_no,$school_info_logo,$student_name,$student_class,$student_roll_no,$student_date_of_birth,$student_father_contact_number,$student_photo,$student_image,$roll_no,$school_roll_no,$school_info_school_code,$student_gender,$school_info_school_address,$path1;
...
$que1="select * from student_admission_info where student_roll_no='$roll_no[$j]' and session_value='$session1'";
    ...
    while($row1=mysqli_fetch_assoc($run1)){
	 ...
	 $student_mother_photo=$row1['student_mother_image_name'];

	 
	 $student_photo1=$_SESSION['amazon_file_path']."student_documents/".$student_photo;$student_photo1=str_replace(" ","%20",$student_photo1);
     $student_mother_photo1=$_SESSION['amazon_file_path']."student_documents/".$student_mother_photo;$student_mother_photo1=str_replace(" ","%20",$student_mother_photo1);
}
if($horizontal>2 && $vertical>1){
  ...
  $width=$width1*$horizontal;
	...
	$this->Image('../id_card_image/id_card_background_header_1.png',10+$width,9+$height,80,13.5);
		...
		$this->Image('../../../images/blank_logo.png',10+$width,10+$height,11,12,'png');
		}else{
		    
$ext2 = explode('.',$school_info_logo);
$ext = end($ext2);
		$this->Image($path1,10+$width,9+$height,11,13,$ext);
		}
		
		
		
		if($student_photo==null){
		$this->Image('../../../images/blank.jpg',10+$width,23+$height,14.6,14.6,'jpg');
		}else{
		$stu_ext2 = explode('.',$student_photo);
        $stu_ext = end($stu_ext2);
		$this->Image($student_photo1,10+$width,23+$height,14.6,14.6,$stu_ext);
		}	
		
		
        
		if($student_mother_photo==null){
		$this->Image('../../../images/blank.jpg',75+$width,23+$height,14.6,14.6,'jpg');
		}else{
		$stu_mother_ext2 = explode('.',$student_mother_photo);
        $stu_mother_ext = end($stu_mother_ext2);
		$this->Image($student_mother_photo1,75+$width,23+$height,14.6,14.6,$stu_mother_ext);
		}
...
if($horizontal>0)
  ...
  $this->SetLeftMargin(10+$width);  
		...
		$this->Cell(50,15,'Mother ID Card',1,0,'C');
		$this->Cell(15,15,'',1,0,'C');
		$this->Ln();
		//--------------------------------------------
		$this->Cell(52,0,'',0,0,'C');
		...
		$this->SetFont('Times','B',9);
		$this->SetTextColor(3,3,3);
		$this->Cell(40,4,'Mn :',0,0,'L');
		$this->Cell(40,4,''.$student_mother_name,0,0,'R');
		$this->Ln();
		//--------------------------------------------
		$this->SetFont('Times','B',9);
		$this->SetTextColor(3,3,3);
		...
		$this->Cell(40,4,''.$student_mother_contact_number,1,0,'R');
		$this->Ln();
