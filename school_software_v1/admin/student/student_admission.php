     ...
     var  stream_name=document.getElementById('student_class_stream').value;
       ...
       $.ajax({
              ...
              success: function(detail1){
                  //prompt("Copy to clipboard: Ctrl+C, Enter", detail1);
...
function for_route(){
    ...
    $('#student_bus_route').html("<option value=''>Loading.....</option>");
    if(bus_fee_category_name!=''){
        $.ajax({
        type: "POST",
        url: access_link+"student/ajax_get_student_bus_route_list.php?bus_fee_category_name="+bus_fee_category_name+"",
        success: function(detail){
        $('#student_bus_route').html(detail);
        }
        });
    }else{
        $("#student_bus_route").html("<option value=''>Select</option>");
    }
    for_fees();
}
...
function for_stop(){
    ...
    $('#bus_fee_category_name').html("<option value=''>Loading.....</option>");
    if(student_bus=='Yes'){
        $.ajax({
        type: "POST",
        url: access_link+"student/ajax_get_student_bus_stop_list.php",
        success: function(detail){
        $('#bus_fee_category_name').html(detail);
        }
        });
    }else{
        $("#bus_fee_category_name").html("<option value=''>Select</option>");
    }
}

function for_no(){
    var student_bus_route=document.getElementById('student_bus_route').value;
    var bus_fee_category_name=document.getElementById('bus_fee_category_name').value;
...
function for_fees(){
    ...
    var bus_fee_category_name=document.getElementById('bus_fee_category_name').value;
    var student_class=document.getElementById('student_class').value;
    if(student_bus=='Yes' && bus_fee_category_name!='' && student_class!=''){
        var category_name=bus_fee_category_name.split('|?|');
        $.ajax({
        type: "POST",
        url: access_link+"student/ajax_get_student_bus_fees.php?category_code="+category_name[1]+"&student_class="+student_class+"",
        success: function(detail){
        $('#student_bus_fees').val(detail);
        }
        });
    }else{
        $("#student_bus_fees").val("0");
    }
}
</script>
...
<script>
function myFunction() {
    var school_name=document.getElementById('sms_school_name').value;
    var student_name=document.getElementById('student_name').value;
   // var sms_cont="Dear Parents, Thanks for choosing us for a bright future of "+student_name+" Regards - "+school_name+"[SCHOOL]";
    
     var sms_cont="Dear Student , Your admission is confirmed at "+student_name+". Regards,"+school_name+".[SIMPTION]";
   
    $("#sms_content").val(sms_cont);
    ...
    $("#my_form").submit(function(e){
        ...
        $.ajax({
            ...
            success: function(detail){
                ...
                if (res[1] === 'success') {
                    alert_new('Record updated successfully', 'green');
...
function search_admission_no(number,roll)
{

$.ajax({
    type: "POST",
    url: access_link+"student/ajax_search_admission_no.php?number="+number+"&roll="+roll,
    cache: false,
    success: function(detail){
        if (detail === 'Yes') {
            alert_new('Admission Number already exists', 'red');
        }
    }
});
 ...
 <li><a href="javascript:get_content('index_content')"><i class="fa fa-dashboard"></i> Home</a></li>
	...
	$student_class_subject=$row['student_class_subject'];
	...
	$previous_school_tc_no=$row['previous_school_tc_no'];
	$previous_school_tc_date=$row['previous_school_tc_date'];
	$mother_qualification=$row['mother_qualification'];
	$house_name1=$row['house_name'];
	
	//$previous_school_tc_date=date('d-m-Y',strtotime($previous_school_tc_date1));
	...
	$student_fatehr_annual_income = $row['student_fatehr_annual_income'];
	$caste_certificate_no = $row['caste_certificate_no'];
	$student_blood_group = $row['student_blood_group'];
	$student_identity_category = $row['student_identity_category'];
	$house_name = $row['house_name'];
	$house_name1 = $house_name;
     if($house_name==''){
         $house_name1=$student_identity_category;
     }
	...
	$student_image=$row['student_image_name'];
	 $student_father_image=$row['student_father_image_name'];
	$student_mother_image=$row['student_mother_image_name'];
	$student_guardian_image=$row['student_guardian_image_name'];
	$student_tc_image=$row['student_tc_image_name'];
	$student_last_marksheet_image=$row['student_last_marksheet_image_name'];
	$student_income_certificate_image=$row['student_income_certificate_image_name'];
	$student_cast_certificate_image=$row['student_cast_certificate_image_name'];
	$student_dob_image=$row['student_dob_image_name'];
	$student_adhar_card_image=$row['student_adhar_card_image_name'];
	$student_sssmid_image=$row['student_sssmid_image_name'];
	
	$student_sssmid_image=$row['student_sssmid_image_name'];
	$student_other_document1_image=$row['student_other_document1_image_name'];
	$student_other_document2_image=$row['student_other_document2_image_name'];

	}
	$auto_admission_number_status=0;
		if($_SESSION['database_name1']=='msnpschoolgopalganj'){
	if($student_admission_number==''){
		$que1="select * from student_admission_info where student_roll_no='$student_roll_no' and session_value='$session1' and student_status='Deactive' and registration_final='no'";
   ...
   if(mysqli_num_rows($run1)>0){
	   ...
	   $admission_no=$row11['admission_no'];  
	   $admission_no=$admission_no+1;
	   $student_admission_number="MS/".$admission_no;
	   $auto_admission_number_status=1;
	   }  
   }
	
	}
		}		if($_SESSION['database_name1']=='bharatbharatischoolkullu'){
	if($student_admission_number==''){
		$que1="select * from student_admission_info where student_roll_no='$student_roll_no' and session_value='$session1' and student_status='Deactive' and registration_final='no'";
    $run1=mysqli_query($conn73,$que1);
   if(mysqli_num_rows($run1)>0){
	   ...
	   $admission_no=$row11['admission_no'];  
	   $admission_no=$admission_no+1;
	   $student_admission_number=$admission_no;
	   $auto_admission_number_status=1;
	   }  
   }
...
<!------------------------------------------------Start Registration form--------------------------------------------------->
		...
		<form name="myForm" method="post" enctype="multipart/form-data" action="" id="my_form">
		
			<input type="hidden" name="auto_admission_number_status" value="<?php echo $auto_admission_number_status; ?>">	
			<input type="hidden" name="auto_admission_number" value="<?php echo $admission_no; ?>">	
            <div class="box-body ">
			<h3 style="color:#d9534f;"><b><?php echo $language['Personal Detail']; ?>:</b></h3>
			    <div class="col-md-3 ">
						...
						<div class="form-group">
						  <label><?php echo $language['Student Name']; ?></label>
						   <input type="text" name="student_name" id="student_name" value="<?php echo $student_name; ?>" oninput="myFunction();" class="form-control">
				...
				<div class="col-md-3 ">
						<div class="form-group">
						  <label><?php echo "Father's Qualification"; $language['Father Name']; ?></label>
						   <input type="text"  name="father_qualification"   value="<?php echo $father_qualification; ?>" class="form-control">
						</div>
				</div>
				...
				<div class="col-md-3 ">
						<div class="form-group">
						  <label><?php echo "Mother Qualification";//$language['Mother Name']; ?></label>
						   <input type="text"  name="mother_qualification"   value="<?php echo $mother_qualification; ?>" class="form-control">
						</div>
				</div>
				...
				<div class="col-md-3 ">	
					<div class="form-group">
					  ...
					  <option <?php if($student_category=='General') { echo 'selected'; } ?> value="General">General</option>
					  <option <?php if($student_category=='OBC') { echo 'selected'; } ?> value="OBC">OBC</option>
					  <option <?php if($student_category=='SC') { echo 'selected'; } ?> value="SC">SC</option>
					  <option <?php if($student_category=='ST') { echo 'selected'; } ?> value="ST">ST</option>
					  <option <?php if($student_category=='Other') { echo 'selected'; } ?> value="Other">Other</option>
					  </select>
					</div>
				</div>	
				<div class="col-md-3 ">	
				...
				<div class="col-md-3">
						<div class="form-group">
						  <label><?php echo $language['Date Of Birth(word)']; ?></label>
						   <input type="text"  id="student_date_of_birth_in_word1" name="student_date_of_birth_in_word"  value="<?php echo $student_date_of_birth_in_word; ?>" class="form-control" >
				        </div>
				</div>
				
				<div class="col-md-3">
						<div class="form-group">
						  <label><?php echo"student_cast";//$language['student_cast']; ?></label>
						   <input type="text"  id="student_caste" name="student_caste"  value="<?php echo $student_caste; ?>" class="form-control">
				        </div>
				</div>
				...
				<div class="col-md-3">	
					<div class="form-group" >
					    <label><?php echo "House Name"; ?></label>
					    <select name="house_name"  id="house_name" class="form-control">
						       <option value=""><?php echo "Select House Name"; ?></option>
						       <?php 
						       $query1234 = "select identity_category_name from school_info_identity_category"; 
						       $rest1234 = mysqli_query($conn73,$query1234);
						       while($row1234=mysqli_fetch_assoc($rest1234)){
						          	$house_name01=$row1234['identity_category_name']   ?>
						       <option <?php if($house_name1==$house_name01){ echo 'selected'; } ?> value="<?php echo $house_name01; ?>"><?php echo $house_name01; ?></option>
					     <?php } ?>
					    </select>
					</div>
				</div>
				...
				<div class="col-md-3">
						<div class="form-group">
						  <label><?php echo"Blood Group";//$language['student_cast']; ?></label>
						   <input type="text"  id="student_blood_group" name="student_blood_group"  value="<?php echo $student_blood_group; ?>" class="form-control">
				        </div>
				</div>
			...
			<div class="box-body">
			    ...
			    <div class="col-md-3">
						<div class="form-group">
						  ...
						  <input type="text" minlength="13" maxlength="13" name="student_adhar_number" value="<?php echo $student_adhar_number; ?>" class="form-control">
						</div>
				</div>
				<div class="col-md-3">
						<div class="form-group">
				...
				<div class="col-md-3">		
						<div class="form-group">
						  <label><?php if($_SESSION['software_link']=='amarawaticollege'){echo "Board Ragistration";}else{ echo $language['Sssm Id No']; } ?></label>
				...
				<div class="col-md-3">				
					<div class="form-group">
					  <label ><?php echo $language['Child Id']; ?>/Child UID</label><br>
				...
				<div class="col-md-3">				
					<div class="form-group" >
					  <label ><?php echo "Annual Income"; ?></label>
					  <input type="text"  name="annual_income"   value="<?php echo $student_fatehr_annual_income; ?>" class="form-control">
					</div>
				</div>
				
				<div class="col-md-3">				
					<div class="form-group" >
					  <label ><?php echo "Cast certificate No"; ?></label>
					  <input type="text"  name="cast_certifictae_no"   value="<?php echo $caste_certificate_no; ?>" class="form-control">
					</div>
				</div>
			...
			<h3 style="color:#d9534f;"><b><?php echo $language['Admission Details']; ?>:</b></h3>
				...
				<div class="col-md-3 ">
						<div class="form-group">
						   ...
						   <select name="student_admission_class" class="form-control">
						       ...
						       <option value="<?php echo $class_name; ?>" <?php if($class_name==$student_admission_class){ echo 'selected'; }?>><?php echo $class_name; ?></option>
					           <?php } ?>
						   </select>
				...
				<div class="col-md-3 " style="<?php if($_SESSION['database_name1']=='livingstonenagaland'){ echo 'display:none'; } ?>">
						<div class="form-group">
						   ...
						   <input type="hidden" name="student_date_of_admission11"   value="<?php echo $student_date_of_admission; ?>" class="form-control">
						</div>
				</div>
				...
				<div class="col-md-3 ">
						<div class="form-group">
						<label><?php if($_SESSION['software_link']=='amarawaticollege'){echo "Previous Roll No. ";}else{ echo $language['Admission No'];  } ?></label>
						<input type="text"  name="student_admission_number"    value="<?php echo $student_admission_number; ?>" <?php if($_SESSION['software_link']=='successschooljagdalpur'){ echo "onblur='search_admission_no(this.value,$student_roll_no)' ";  }?>class="form-control">
				        </div>
			    </div>			
			    <div class="col-md-3" style="<?php if($_SESSION['database_name1']=='progressiveheights'){ echo 'display:none'; } ?>">	
					<div class="form-group" >
			    ...
			    <div class="col-md-3 ">
					<div class="form-group">
					  <label><?php echo "Previous School Tc No.";//$language; ?></label>
					   <input type="text"  name="previous_school_tc_no"    value="<?php echo $previous_school_tc_no; ?>" class="form-control">
					</div>
			    </div>

               <div class="col-md-3 ">
					<div class="form-group">
					  <label><?php echo "Previous School Tc Date.";//$language; ?></label>
					   <input type="date"  name="previous_school_tc_date"    value="<?php echo $previous_school_tc_date; ?>" class="form-control">
					</div>
			    </div>			    
				...
				<div class="col-md-3 " id="student_class_stream_div">				
					<div class="form-group">
					  <label >Stream<font style="color:red"><b>*</b></font></label>
					    <select class="form-control" name="student_class_stream" id="student_class_stream" onchange="get_group(this.value);change_stream();get_group_subject();" >
						<?php if($student_class_stream==''){ ?>
						 <option value="">Select Stream</option> <?php } else { ?>
						       ...
						       <?php } $que="select stream_name from school_info_stream_info";
                               $run=mysqli_query($conn73,$que);
                               while($row=mysqli_fetch_assoc($run)){
				...
				<div class="col-md-8" id="student_class_group_subject_div" style="display:none;">				
					<div class="form-group">
					 ...
					 <input type="text" name="student_class_group_subject" id="student_class_group_subject" value="<?php echo $student_class_subject; ?>" class="form-control new_student" readonly>
					</div>
				</div>
				<div class="col-md-3 ">	
					<div class="form-group" >
				...
				<div class="col-md-3" style="<?php if($fees_type=='fees1'){ echo 'display:none;'; } ?>">				
					<div class="form-group" >
					  ...
					  <input type="text" name="student_registration_fee" id="student_registration_fee" placeholder="<?php echo $language['Registration Fees']; ?>"  value="<?php echo $student_registration_fee; ?>" class="form-control">
					</div>
				</div>
				...
				<div class="col-md-3">				
					<div class="form-group" >
					  ...
					  <select class="form-control"  name="student_bus" id="student_bus" onchange="for_stop();">
				...
				<div class="col-md-3" id="div_bus_fee_ctgry_name" style="<?php //if($student_bus=='No'){ echo 'display:none'; } ?>">				
					<div class="form-group">
					  <label>Bus Fee Category</label>
					    <select class="form-control select2" name="bus_fee_category_name" id="bus_fee_category_name" onchange="for_route();" >
					           <option  value="">Select</option>
						       <?php
				...
				<div class="col-md-3">				
                    ...
                    <select class="form-control select2" name="student_bus_route" id="student_bus_route" onchange="for_no();" >
                    <option value="">Select</option>
                    <?php
                    //if($student_bus=='Yes'){
                    ...
                    <input type="hidden" name="student_bus_no" id="student_bus_no" value="<?php echo $student_bus_no; ?>" class="form-control" />
					</div>
				</div>
				...
				<div class="col-md-3">				
					<div class="form-group" >
					  ...
					  <input type="text" name="student_bus_fees" id="student_bus_fees" value="" class="form-control" readonly />
					  </div>
				</div>
				...
				<div class="col-md-3 ">				
					<div class="form-group" >
					  ...
					  <?php } ?>
					        <?php
							$qry="select student_roll_no,student_name,student_class,student_class_section,student_father_name,student_father_contact_number from student_admission_info where student_status='Active' and session_value='$session1' ";
				...
				<div class="col-md-3 ">				
					...
					<div class="form-group" style="display:none;">
					  <label>Sibling Unique ID (1)</label>
				...
				<div class="col-md-3 " style="display:none;">				
					<div class="form-group" >
					  <label>Sibling Unique ID (2)</label>
		...
		<div class="box-body ">
				...
				<div class="col-md-3 ">
						...
						<div class="form-group">
						  <label><?php echo $language['Father Contact No2']; ?></label>
						   <input type="text" name="student_father_contact_number2" value="<?php echo $student_father_contact_number2; ?>" class="form-control">
						</div>
				</div>
				<div class="col-md-3 ">
						<div class="form-group">
		    ...
		    <div class="box-body ">
				...
				<div class="col-md-1">	
					<div class="form-group">
					 <img onclick="open_file1('student_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_student_photo" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>
				<div class="col-md-2 ">	
					<div class="form-group" > 
				...
				<div class="col-md-1">	
					<div class="form-group">
					  <img onclick="open_file1('student_father_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_father_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_father_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_father_photo" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>
				<div class="col-md-2 ">	
					<div class="form-group" > 
				...
				<div class="col-md-1">	
					<div class="form-group">
					  <img onclick="open_file1('student_mother_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_mother_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_mother_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_mother_photo" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>
				<div class="col-md-2">	
					<div class="form-group">
				...
				<div class="col-md-1">	
					<div class="form-group" >
					  <img onclick="open_file1('student_guardian_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_guardian_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_guardian_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_guardian_photo" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>
			    <div class="col-md-2 ">
						<div class="form-group">
				...
				<div class="col-md-1">	
					<div class="form-group" >
					  <img onclick="open_file1('student_last_marksheet_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_last_marksheet_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_last_marksheet_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_student_last_marksheet" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>
				<div class="col-md-2 ">
						<div class="form-group">
			...
			<div class="col-md-1">	
					<div class="form-group" >
					  <img onclick="open_file1('student_tc_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_tc_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_tc_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_student_tc" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>
				<div class="col-md-2">
						<div class="form-group">
			...
			<div class="col-md-1">	
					<div class="form-group" >
					  <img onclick="open_file1('student_income_certificate_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_income_certificate_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_income_certificate_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_student_income_certificate" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>
				<div class="col-md-2 ">
						<div class="form-group">
			...
			<div class="col-md-1">	
					<div class="form-group" >
					  <img onclick="open_file1('student_cast_certificate_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_cast_certificate_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_cast_certificate_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_student_cast_certificate" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>			
				<div class="col-md-2 ">	
					<div class="form-group" >
			...
			<div class="col-md-1">	
					<div class="form-group" >
					  <img onclick="open_file1('student_dob_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_dob_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_dob_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_student_dob" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>
							<div class="col-md-2 ">	
					<div class="form-group" > 
				...
				<div class="col-md-1">	
					<div class="form-group" >
					  <img onclick="open_file1('student_adhar_card_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_adhar_card_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_adhar_card_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_adhar_photo" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>
				...
				<div class="col-md-2 ">	
					<div class="form-group" > 
					  ...
					  <input type="file"  id="student_sssmid_image" name="student_sssmid_image"  onchange="check_file_type(this,'student_sssmid_image','show_sssmid_photo','image');" value="" class="form-control" accept=".gif, .jpg, .jpeg, .png">
					</div>
				</div>
				
				<div class="col-md-1">	
					<div class="form-group" >
					  <img onclick="open_file1('student_sssmid_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_sssmid_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_sssmid_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_adhar_photo" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>
					...
					<div class="form-group" > 
					  ...
					  <input type="file"  id="student_other_document1_image" name="student_other_document1_image"  onchange="check_file_type(this,'student_other_document1_image','show_student_other_document1_image','image');" value="" class="form-control" accept=".gif, .jpg, .jpeg, .png">
					</div>
				</div>
				
				<div class="col-md-1">	
					<div class="form-group" >
					  <img onclick="open_file1('student_other_document1_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_other_document1_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_other_document1_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_student_other_document_image" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>
					<div class="col-md-2 ">	
					<div class="form-group" > 
					  <label>Other Document-2</label>
					  <input type="file"  id="student_other_document2_image" name="student_other_document2_image"  onchange="check_file_type(this,'student_other_document2_image','show_student_other_document2_imagee','image');" value="" class="form-control" accept=".gif, .jpg, .jpeg, .png">
					</div>
				</div>
				
				<div class="col-md-1">	
					<div class="form-group" >
					  <img onclick="open_file1('student_other_document2_image','<?php echo $student_roll_no; ?>');" src="<?php if($student_other_document2_image!=''){ echo $_SESSION['amazon_file_path']."student_documents/".$student_other_document1_image; }else{ echo $school_software_path."images/student_blank.png"; }  ?>" id="show_student_other_document2_image" height="50" width="50" style="margin-top:10px;">
					</div>
				</div>
				...
				<div class="col-md-3 ">
						<div class="form-group">
						  <label><?php if($_SESSION['database_name1']=='gyanjyotiacademyshamshabad'){echo "Vision"; }elseif($_SESSION['software_link']=='amarawaticollege'){echo "Subject's";}else{echo $language['Remark 1'];} ?></label>
						   <input type="text"  name="student_remark_1"  value="<?php echo $student_remark_1; ?>" class="form-control">
						</div>
			    </div>
				<div class="col-md-3">
					<div class="form-group">
					  <label><?php if($_SESSION['database_name1']=='gyanjyotiacademyshamshabad'){echo "D.Hygiene"; }elseif($_SESSION['software_link']=='amarawaticollege'){echo "Passing Year";}else{echo $language['Remark 2'];} ?></label>
					   <input type="text"  name="student_remark_2"   value="<?php echo $student_remark_2; ?>" class="form-control">
					</div>
			    </div>
				<div class="col-md-3">
					<div class="form-group">
					  <label><?php if($_SESSION['database_name1']=='gyanjyotiacademyshamshabad'){echo "A.Hygiene"; }else{echo $language['Remark 3'];} ?></label>
					   <input type="text"  name="student_remark_3"   value="<?php echo $student_remark_3; ?>" class="form-control">
					</div>
			    </div>
				<div class="col-md-3 ">
					<div class="form-group">
					  <label><?php if($_SESSION['database_name1']=='gyanjyotiacademyshamshabad'){echo "Specific Ailment If Any"; }else{echo $language['Remark 4'];} ?></label>
                ...
                <div class="col-md-12 ">
                <?php
                $qry="select school_info_school_name from school_info_general";
                $rest=mysqli_query($conn73,$qry);
                while($row22=mysqli_fetch_assoc($rest)){
                ...
                ?>
                	...
                	<?php
				if($_SESSION['database_name1']=='wisdomipshdr' || $_SESSION['database_name1']=='gyanbhartischoolkabrai' || $_SESSION['database_name1']=='stteresakpl'){
                ...
                <input type="hidden" name="sms_school_name" id="sms_school_name" value="<?php echo $school_info_school_name; ?>" class="form-control" />
                <div class="col-md-8 ">	
                ...
                <input type="text" name="sms_content" id="sms_content" value="Dear Parents, Thanks for choosing us for a bright future of <?php echo $student_name; ?> Regards - <?php echo $school_info_school_name; ?>[SCHOOL]" class="form-control" />
                </div>
                </div>
                </div>
