...
function for_route(ser_no) {
    ...
    if (student_bus === 'Yes') {
        $.ajax({ type: "POST", url: access_link + "student/ajax_get_student_bus_route_list.php",
            success: function(d) { $('#student_bus_route_' + ser_no).html(d); } });
    } else {
...
function for_stop(ser_no) {
    var route = document.getElementById('student_bus_route_' + ser_no).value;
    $('#bus_fee_category_name_' + ser_no).html("<option value=''>Loading...</option>");
    if (route !== '') {
        $.ajax({ type: "POST", url: access_link + "student/ajax_get_student_bus_stop_list.php",
            success: function(d) { $('#bus_fee_category_name_' + ser_no).html(d); } });
    } else {
...
function for_no(ser_no) {
    var route    = document.getElementById('student_bus_route_' + ser_no).value;
    var stop     = document.getElementById('bus_fee_category_name_' + ser_no).value;
    if (route !== '' && stop !== '') {
        var cat = stop.split('|?|');
        $.ajax({ type: "POST",
            url: access_link + "student/ajax_get_student_bus_no_list.php?stop_name=" + cat[0] + "&student_bus_route=" + route,
            success: function(d) { $('#student_bus_no_' + ser_no).val(d); } });
    } else {
...
$("#my_form").submit(function(e) {
    ...
    $.ajax({
        ...
        success: function(detail) {
            ...
            if (res[1] === 'success') {
                alert_new('Profile updated successfully', 'green');
            } else {
...
<section class="si-ph">
  <div class="si-ph-left">
    <div class="si-ph-badge"><i class="fa fa-user-circle"></i></div>
    <div>
      <h1 class="si-ph-title">Student Profile Update</h1>
  ...
  <nav class="si-ph-bc">
    ...
    <span class="si-bc-sep"><i class="fa fa-chevron-right"></i></span>
    <span class="si-bc-active"><i class="fa fa-user-circle"></i> Profile Update</span>
  </nav>
</section>
    ...
    <div class="st-filter-card">
      ...
      <div class="st-filter-body">
        ...
        <div class="st-filter-group">
          ...
          <select class="form-control" name="" id="student_class_stream" onchange="for_search11();">
            <option value="All">All</option>
            <?php
            $q19 = "select stream_name from school_info_stream_info where stream_name!=''";
            $r19 = mysqli_query($conn73, $q19) or die(mysqli_error($conn73));
        ...
        <div class="st-filter-group">
          ...
          <select class="form-control" name="student_fee_category_code" id="student_fee_category_code" onchange="for_search11();">
            ...
            while ($row01 = mysqli_fetch_assoc($run01)) {
                echo '<option value="' . htmlspecialchars($row01['category_code']) . '">' . htmlspecialchars($row01['category_name']) . '</option>';
            }
            ?>
          </select>
        </div>
        ...
        <div class="st-filter-group">
          ...
          <select name="order_by" id="order_by" class="form-control" onchange="for_search11();">
            <option value="">Select</option>
            <option value="order by CAST(school_roll_no AS UNSIGNED) asc">Roll No.</option>
          </select>
        </div>
