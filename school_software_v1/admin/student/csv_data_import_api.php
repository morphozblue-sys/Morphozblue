...
$p=0;
   ...
   if($v==0){
	...
	$update_condition_value=",student_class_section ='A'";
					 ...
					 $insert_que_value=$insert_que_value.",'".my_validation($line[$i])."'";
					 $update_que_column=$update_que_column.",".$column_name[$i]."='".my_validation($line[$i])."'";
        ...
        $c=0;     
if($b>1){
         $prevQuery = "SELECT s_no FROM school_info_rfid_card $final_where_update";
                $prevResult =mysqli_query($conn73,$prevQuery);
		$c=mysqli_num_rows($prevResult);
}
                if($c>0){
	 $update_que="UPDATE school_info_rfid_card SET $update_que_column $final_where_update";
    ...
    $student_roll_no=$y.$class_roll_new;
         ...
         $insert_que="INSERT INTO student_admission_info (student_roll_no,student_id_generate,stuent_old_or_new,registration_final,$insert_que_column,$condition_column) VALUES ('$student_roll_no','$student_id_generate','Old','yes',$insert_que_value,$condition_value)";
  ...
  function date_of_birth($dob){
	...
	$dob_final=$dob_slash[0]."-".$dob_slash[1]."-".$dob_slash[2];
	 ...
	 }else if($dob_dot_count>1){
	 if($dob_dot[0]>1900){
	 $dob_final=$dob_dot[0]."-".$dob_dot[1]."-".$dob_dot[2];
	 }else if($dob_dot[2]>1900){
	 $dob_final=$dob_dot[2]."-".$dob_dot[1]."-".$dob_dot[0];
	 }else{
	   if($dob_dot[2]>40 && $dob_dot[2]< 99){
	          $dob_final='19'.$dob_dot[2]."-".$dob_dot[1]."-".$dob_dot[0];
	     }else if($dob_dot[2]<=40){
	         $dob_final='20'.$dob_dot[2]."-".$dob_dot[1]."-".$dob_dot[0]; 
	     }else{
	 $dob_final='';
	     }
	 }
