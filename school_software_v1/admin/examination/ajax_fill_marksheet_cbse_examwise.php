...
$exam_table=$exam_term."_";
 ...
 $exam_marks_maximum1=$exam_table.$exam_type."_maximum_mark";
$exam_marks_minimum1=$exam_table.$exam_type."_minimum_mark";
 $exam_marks_add1=$exam_table.$exam_type."_mark_add";
 ...
 $query5="select * from school_info_subject_info where class='$student_class' and session_value='$session1'$condition_0011$filter37";
...
<tr>
  ...
  <?php } ?>
  <th style="<?php if($_SESSION['examination_fill_marks_total_day_field']!='yes'){echo 'display:none'; } ?>">Total Days</th>
  <th style="<?php if($_SESSION['examination_fill_marks_prasent_day_field']!='yes'){echo 'display:none'; } ?>">Total Present</th>
  <th style="<?php if($_SESSION['examination_fill_marks_remarks_field']!='yes'){echo 'display:none'; } ?>">Remark</th>
...
<td>
<select name="<?php echo 'student_mark_add_'.$subject_code[$aw0]; ?>" id="" style="width:60px;" required>
...
<?php for($aw0=0;$aw0<$total_subject;$aw0++){ ?>
<td><input type="text" name="<?php echo 'student_maximum_marks_'.$subject_code[$aw0]; ?>" value="<?php echo $student_maximum_marks[$aw0]; ?>" id="<?php echo 'student_maximum_marks_'.$subject_code[$aw0]; ?>" style="width:60px;"></td>
<?php } ?>
<td>&nbsp;</td>
<td>&nbsp;</td>
</tr>
...
<td><input type="text" name="<?php echo 'student_minimum_marks_'.$subject_code[$aw01]; ?>" value="<?php echo $student_minimum_marks[$aw01]; ?>" id="" style="width:60px;"></td>
<?php } ?>
<td>&nbsp;</td>
<td>&nbsp;</td>
</tr>
...
while($row3=mysqli_fetch_assoc($res3)){ //echo '<pre>';print_r($row3);
...
<?php if($_SESSION['software_link']!='shiningstarsiwan'){ echo $school_roll_no; }else{echo $student_admission_number; } ?>
</td>
<?php for($aw1=0;$aw1<$total_subject;$aw1++){ ?>
<td><input type="text" name="<?php echo 'student_marks_'.$student_roll_no.'_'.$subject_code[$aw1]; ?>" value="<?php echo $subjectwise_student_marks[$aw1]; ?>" id="<?php echo 'id_'.$student_roll_no.'_'.$subject_code[$aw1]; ?>" oninput="for_validation(this.id,this.value,'<?php echo $subject_code[$aw1]; ?>');" style="width:60px;"></td>
<?php } ?>
<td style="<?php if($_SESSION['examination_fill_marks_total_day_field']!='yes'){echo 'display:none'; } ?>"><input type="text" name="total_attendance[]" value="<?php echo $total_attendance; ?>"></td>
<td style="<?php if($_SESSION['examination_fill_marks_prasent_day_field']!='yes'){echo 'display:none'; } ?>"><input type="text" name="total_present[]" value="<?php echo $total_present; ?>"></td>
<td style="<?php if($_SESSION['examination_fill_marks_remarks_field']!='yes'){echo 'display:none'; } ?>"><input type="text" name="attendance_remark[]" value="<?php echo $attendance_remark; ?>"></td>
