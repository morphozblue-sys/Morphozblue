...
<script type="text/javascript">
      function for_list(){ 
			...
			if(student_class_section!='' && student_class!='' && exam_type!='' && exam_term!=''){
             ...
             $.ajax({
			  type: "POST",
             url:access_link+"examination/ajax_fill_marksheet_cbse_examwise.php?id="+student_class+"&student_section="+student_class_section+"&student_exam_type="+exam_type+"&exam_term="+exam_term+"&student_class_stream="+student_class_stream+"&student_class_group="+student_class_group+"&select_month="+select_month+"&order_by="+order_by+"&student_limit="+student_limit+"",
              cache: false,
...
window.scrollTo(0, 0);
    $("#loader_div_id").html(loader_div);
        $.ajax({
            url: access_link+"examination/marksheet_fill_cbse_examwise_api.php",
            type: "POST",
            data: formdata,
            ...
            success: function(detail){
			   ...
			   if(res[1]=='success'){
				   ...
				   ////alert_new(detail);
				   //get_content('examination/marksheet_fill_cbse_examwise');
				   for_list();
				   $("#loader_div_id").html("");
            }
			}
...
if($stream_name!=''){
		...
		<div class="col-md-3" >				
		  ...
		  <div class="col-md-12">
		        <center><input type="submit" name="finish" value="<?php echo $language['Submit']; ?>" onclick="return valid();" class="btn  btn-success" /></center>
		  </div>
		  </form>
	      </div>
