...
$category_set=$_POST['category_set'];
...
$attendance_remark11=$exam_type.'_attendance_remark';
 ...
 $exam_marks_add1=$exam_type."_mark_add";
      ...
      if($category_set==1){
         ...
         $student_marks11="";
 $subject_category_code=$_POST['subject_category_code'];
 for($bb=0;$bb<count($subject_category_code);$bb++){
       $student_marks_get[$subject_category_code[$bb]]=$_POST['student_marks'.$subject_category_code[$bb]];
      // var_dump($student_marks_get[$subject_category_code[$bb]]);
   }  
 }
...
$exam_table="examination";    
 ...
 for($i=0; $i<$count; $i++){
     if($category_set==1){
       ...
       $exam_marks_maximum_array="";
       $exam_marks_mimimum_array="";
 $student_mark_add_array="";
 for($bb=0;$bb<10;$bb++){
   if (in_array($bb, $subject_category_code)){
       $student_marks_array=$student_marks_get[$bb];
       $student_marks11=$student_marks11.$student_marks_array[$i];
       $exam_marks_maximum_array=$exam_marks_maximum_array.$student_maximum_marks[$bb];
       $exam_marks_mimimum_array=$exam_marks_mimimum_array.$student_minimum_marks[$bb];
 $student_mark_add_array=$student_mark_add_array.$student_mark_add[$bb];
   }
    $student_marks11=$student_marks11."|?|";

       $exam_marks_maximum_array=$exam_marks_maximum_array."|?|";
       $exam_marks_mimimum_array=$exam_marks_mimimum_array."|?|";
 $student_mark_add_array=$student_mark_add_array."|?|";
 }
 
 
         
    
 echo $quer="update $exam_table set $exam_marks1='$student_marks11',$total_attendance11='$total_attendance[$i]',$total_present11='$total_present[$i]',$attendance_remark11='$attendance_remark[$i]',$update_by_update_sql  where student_roll_no='$student_roll_no[$i]' and session_value='$session1'";
