...
$query2="select * from student_admission_info where student_class='$student_class'$condition_section and student_class_group='$student_class_group' and student_class_stream='$student_class_stream' and student_status='Active' and session_value='$session1'";    
$total_student=0;        
...
$que="select * from school_info_subject_info where stream_name='$student_class_stream' and group_name='$student_class_group' and class='$student_class'$condition and (session_value='$session1' || session_value='') $filter37";
...
$grand_total_maximum=$row1[$exam_term.'_'.$exam_type.'_maximum_mark']+$grand_total_maximum;
$exam_marks_highest_award[$total_subject]=0;
$exam_marks_highest_award_total=0;
$total_subject++;
}
$percent_common_total=0;
for($ac=0;$ac<$total_student;$ac++){
$query1="select * from examination_$exam_term where student_roll_no='$student_roll_no[$ac]' and session_value='$session1'";
...
$exam_marks3=$row11[$exam_type."_".$subject_code[$i]."_marks"];
$grand_total_obtained=$exam_marks3+$grand_total_obtained;
if($exam_marks3>=$exam_marks_highest_award[$i]){
$exam_marks_highest_award[$i]=$exam_marks3;

}
if($grand_total_obtained>=$exam_marks_highest_award_total){
$exam_marks_highest_award_total=$grand_total_obtained;  
}
}
}
...
$percentage_common=$grand_total_obtained*100/$grand_total_maximum;
$exam1[$student_roll_no[$ac]]=$grand_total_obtained*100/$grand_total_maximum;
$exam1_roll[$student_roll_no[$ac]]=$student_roll_no[$ac];
}else{
$percentage_common=0;
$exam1[$student_roll_no[$ac]]=$grand_total_obtained;
$exam1_roll[$student_roll_no[$ac]]=$student_roll_no[$ac];
}
$percent_common_total=$percentage_common+$percent_common_total;
}

arsort($exam1);
$exam1_rank_ser=0;
//$exam1_rank='';
foreach ( $exam1 as $key=>$val ){
$exam1_rank_ser++;
$exam1_rank[$key]=$exam1_rank_ser;
}
...
?>
		...
		<div class="col-md-12">
			  ...
			  <div class="col-md-6">
				  <center><button type="button" class="btn btn-success" onclick="exportTableToExcel('printTable', '<?php if($student_section!=''){ echo $student_class.'_'.$student_section.'_'.$exam_term.'_Marks'; }else{ echo $student_class.'_'.$exam_term.'_Marks'; } ?>')"><i class="fa fa-print" aria-hidden="true"></i>  Print In Excel</button></center>
			  </div>
			  <div class="col-md-6">
        ...
        <div class="col-md-12">
          ...
          <div class="box <?php echo $box_head_color; ?>" >
			...
			$exam_query="select * from school_info_exam_types_cbse where exam_code='$exam_type' and session_value='$session1' $filter37";
			$exam_result=mysqli_query($conn73,$exam_query)or die(mysqli_error($conn73));
			while($exam_row=mysqli_fetch_assoc($exam_result)){
		    $exam_type1=$exam_row['exam_type'];
			}	
			?>
			  ...
			  <tr>
			  <td>     
			  <span><center><b>Exam : <?php echo $exam_type1; ?></b></center></span>
			  </td>
			  <td>
			  ...
			  <?php } ?>
              <td>     
			  <center><b>Class : <?php if($student_section!=''){ echo $student_class.'-'.$student_section; }else{ echo $student_class; } ?></b></center>
			  </td>
			  <td>
...
$grade='E';    
					...
					<td><?php echo $exam1_rank[$student_roll_no] ?></td>
                ...
                <thead class="my_background_color">
                    <tr>
                        ...
                        <th></th>
                        <?php for($j=0;$j<$total_subject;$j++){ ?>
                        <th><?php echo $exam_marks_highest_award[$j]; ?></th>
                        <?php } 
                        if($total_subject_marks!=0){
                        $highest_percentage=$exam_marks_highest_award_total*100/$total_subject_marks;
                        }else{
                        $highest_percentage=0;    
                        }
                        ?>
                        <th><?php echo $exam_marks_highest_award_total; ?></th>
                        <th><?php echo round($highest_percentage,2).' %'; ?></th>
                        <th></th>
                        <th></th>
                        <th></th>
                    </tr>
