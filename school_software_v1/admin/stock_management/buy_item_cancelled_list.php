...
$.ajax({
type: "POST",
url: access_link+"stock_management/buy_item_delete.php?id="+s_no+"",
cache: false,
...
$.ajax({
type: "POST",
url: access_link+"stock_management/buy_item_cancel.php?id="+s_no+"",
cache: false,
success: function(detail){
    var res=detail.split("|?|");
	    if(res[1]=='success'){
    ...
    <section class="content">
		...
		<div class="box <?php echo $box_head_color; ?>">
            <div class="box-header with-border">
			  <div class="col-md-6"><h4>Books Purchase Cancelled List</h4></div>
			  <div class="col-md-6" style="display:none;"><a href="javascript:get_content('stock_management/add_buy_item')"> <button style="float:right;" type="button" class="btn btn-primary">+ Buy Item</button></a></div>
			</div>
...
<?php
$que="select pd.*, vd.vendor_name from purchase_detail as pd left join vendor_detail as vd on pd.vendor_s_no=vd.s_no where pd.purchase_status='Cancelled' and pd.session_value='$session1' group by pd.invoice_number ORDER BY pd.invoice_number DESC";
...
<tr>
    ...
    <center>
    <a href="javascript:post_content('stock_management/buy_item_edit','id=<?php echo $invoice_number; ?>')" style="color:#fff;"><input type="button" value="Edit" class="btn btn-default" style="background-color:#00a654;color:#fff;display:none;"></a> 
   ...
   <td style="display:none;">
       <a href='<?php echo $pdf_path; ?>stock_invoice_reciept/Purchase_invoice_reciept.php?id=<?php echo $invoice_number; ?>&vid=<?php echo $vendor_s_no;?>' target="_blank"><button type="button" class="btn btn-success" >
