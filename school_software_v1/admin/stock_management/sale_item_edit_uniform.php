...
<script>
function for_detail(student_customer_id1){
    var sales_type=$('input[name=sales_type]:checked').val();
    $('#label_student_customer_id').html(sales_type+'s Name');
    $.ajax({
    type: "POST",
    url: access_link+"stock_management/ajax_get_sales_type_details.php?sales_type="+sales_type+"&student_customer_id1="+student_customer_id1+"",
    cache: false,
...
function item_detail(serial_no){
    ...
    $.ajax({
    type: "POST",
    url: access_link+"stock_management/ajax_get_sales_item_details_edit_uniform.php?item_s_no="+item_s_no+"&invoice_no="+invoice_no+"",
    cache: false,
    success: function(detail){
...
window.scrollTo(0, 0);
    loader();
        $.ajax({
            url: access_link+"stock_management/sale_item_edit_uniform_api.php",
            type: "POST",
            data: formdata,
...
$query12 = "select * from sale_detail_uniform where invoice_number='$invoice_number' and session_value='$session1' and sale_status='Active'";
$result12 = mysqli_query($conn73,$query12) or die(mysqli_error($conn73));
$item_count=0;
while($row12 = mysqli_fetch_assoc($result12)){
$sales_type=$row12['sales_type'];
$student_customer_id1=$row12['student_customer_id'];
$invoice_number=$row12['invoice_number'];
$invoice_date=$row12['invoice_date'];

$item_class1[$item_count]=$row12['item_class'];
$item_category1[$item_count]=$row12['item_category'];
...
?>
   ...
   <section class="content-header">
      ...
      <li><a href="javascript:get_content('stock_management/stock_management')"><i class="fa fa-money"></i> <?php echo $language['Stock Management']; ?></a></li>
        <li class="active">Sale Item Edit</li>
        </ol>
    </section>
	<!---*******************************************************************************************************************************************************************-->
      ...
      <div class="row">
          ...
          <div class="box box-primary my_border_top">
            <div class="box-header with-border ">
              <div class="col-md-4"><h1 class="box-title"><b>Sale Invoice Edit</b></h1></div>
              <div class="col-md-4">
                    <center>
                    <input type="radio" name="sales_type" value="Student" onclick="for_detail('<?php echo $student_customer_id1; ?>');" <?php if($sales_type=='Student'){ echo 'checked'; } ?> > Student &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <input type="radio" name="sales_type" value="Customer" onclick="for_detail('<?php echo $student_customer_id1; ?>');" <?php if($sales_type=='Customer'){ echo 'checked'; } ?> > Customer
                    </center>
              </div>
            ...
            <div class="box-body">
                ...
                <div class="col-md-2">
    			    <div class ="form-group">
                        ...
                        <input type="number" name="invoice_no" class="form-control" id="invoice_no" value="<?php echo $invoice_number; ?>" required <?php if($_SESSION['database_name']!='simptylz_livingstonenagaland'){ echo 'readonly'; } ?> />
                    </div>
                </div>
				    ...
				    <tbody id="item_table">
    				    ...
    				    <tr id="<?php echo 'tr_'.$serial_no; ?>">
    				        <td style="display:none;">
                                <select name="item_class[]" id="<?php echo 'item_class_'.$serial_no; ?>" onchange="for_item('<?php echo $serial_no; ?>');" class="form-control" >
                                ...
                                <option <?php if($item_class1[$i]==$class_name){ echo 'selected'; } ?> value="<?php echo $class_name; ?>"><?php echo $class_name; ?></option>
                                <?php } ?>
                                </select>
    				        </td>
    				        <td>
                                <select name="item_category[]" id="<?php echo 'item_category_'.$serial_no; ?>" onchange="for_item('<?php echo $serial_no; ?>');" class="form-control" required>
                                <option value="">Select</option>
                                <?php
                                ...
                                ?>
                                <option <?php if($item_category1[$i]==$category_s_no){ echo 'selected'; } ?> value="<?php echo $category_s_no; ?>"><?php echo $category_name; ?></option>
                                <?php } ?>
                                </select>
    				        </td>
    				        <td>
        				        
                                <select name="item_name[]" class="form-control select2" id="<?php echo 'item_name_'.$serial_no; ?>" onchange="item_detail('<?php echo $serial_no; ?>');" style="width:100%;" required >
                                <option value="">Select Item</option>
                                <?php
                                $query_v="select nst.*, ns.available_stock from new_stock_item_uniform as nst join new_stock_uniform as ns on ns.item_s_no=nst.s_no where nst.stock_item_status='Active' and ns.new_stock_status='Active'";
                                ...
                                ?>
                                <option <?php if($item_s_no1[$i]==$item_s_no){ echo 'selected'; } ?> value="<?php echo $item_s_no; ?>" <?php if($available_stock<=0){ echo 'disabled'; } ?>><?php echo $item_name.' ['.$item_size.']-['.$item_description.']'; ?></option>
                                <?php } ?>
                                </select>
    				        ...
    				        <td>
    				            ...
    				            <input type='hidden' name='available_stock[]' placeholder='Available Stock' id="<?php echo 'available_stock_'.$serial_no; ?>" class='form-control' step="0.01" />
    				        </td>
    				        <td>
    				        ...
    				        <td>
    					       ...
    					       <?php } echo "<script>item_detail('$serial_no');</script>"; ?>
    				        </td>
    				    </tr>
    				    <?php } ?>
				    </tbody>
				...
				<div class="col-md-12">
    				...
    				<div class="form-group">
                      ...
                      <input type="number" name="total_discount_amount" id="total_discount_amount" value="<?php echo $discount_amount; ?>" oninput="grand_total();" placeholder="Total Discount Amount" class="form-control" step="0.01" />
                    </div>
                    </div>
...
<script>
for_detail('<?php echo $student_customer_id1; ?>');
