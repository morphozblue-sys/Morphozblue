...
$query0="select * from sale_detail_uniform where invoice_number='$invoice_no' and session_value='$session1' and sale_status='Active'";
$res0=mysqli_query($conn73,$query0) or die(mysqli_error($conn73));
while($row0=mysqli_fetch_assoc($res0)){
    $item_s_no=$row0['item_s_no'];
    $quantity=$row0['quantity'];
        $query1="select available_stock from new_stock_uniform where item_s_no='$item_s_no' and new_stock_status='Active'";
        ...
        $query2="update new_stock_uniform set available_stock='$new_available_stock',$update_by_update_sql where item_s_no='$item_s_no' and new_stock_status='Active'";
        mysqli_query($conn73,$query2);
}

$query="update sale_detail_uniform set sale_status='Edited',$update_by_update_sql where invoice_number='$invoice_no' and session_value='$session1' and sale_status='Active'";
...
$quer="insert into sale_detail_uniform(sales_type,student_customer_id,invoice_number,invoice_date,item_class,item_category,item_s_no,quantity,rate,total_amount,total_quantity,grand_total,discount_remark,discount_amount,payable_amount,payment_mode,cheque_bank_name,cheque_date,cheque_number,neft_bank_name,neft_bank_acount_number,sale_status,session_value,$update_by_insert_sql_column) values('$sales_type','$student_customer_id','$invoice_no','$invoice_date','$item_class[$i]','$item_category[$i]','$item_name[$i]','$item_quantity[$i]','$item_rate[$i]','$item_total[$i]','$grand_total_quantity','$item_grand_total','$total_discount_remark','$total_discount_amount','$total_payable_amount','$payment_mode','$cheque_bank_name','$cheque_date','$cheque_no','$neft_bank_name','$neft_bank_account_no','Active','$session1',$update_by_insert_sql_value)";
if(mysqli_query($conn73,$quer)){
    
    $query01="select available_stock from new_stock_uniform where item_s_no='$item_name[$i]' and new_stock_status='Active'";
    ...
    $query1="update new_stock_uniform set available_stock='$new_available_stock',$update_by_update_sql where item_s_no='$item_name[$i]' and new_stock_status='Active'";
