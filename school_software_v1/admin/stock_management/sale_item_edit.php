...
function item_detail(serial_no){
    ...
    $.ajax({
    type: "POST",
    url: access_link+"stock_management/ajax_get_sales_item_details_edit.php?item_s_no="+item_s_no+"&invoice_no="+invoice_no+"",
    cache: false,
    success: function(detail){
...
function for_item(s_no){
    ...
    $.ajax({
	  type: "POST",
	  url: access_link+"stock_management/ajax_get_item_list.php?item_class="+item_class+"&item_category="+item_category+"",
	  cache: false,
	  success: function(detail){
...
window.scrollTo(0, 0);
    loader();
        $.ajax({
            url: access_link+"stock_management/sale_item_edit_api.php",
            type: "POST",
            data: formdata,
...
$query12 = "select * from sale_detail where invoice_number='$invoice_number' and session_value='$session1' and sale_status='Active'";
				...
				<table class="table table-responsive">
				    ...
				    <tbody id="item_table">
    				    ...
    				    <tr id="<?php echo 'tr_'.$serial_no; ?>">
    				        <td>
                                <select name="item_class[]" id="<?php echo 'item_class_'.$serial_no; ?>" onchange="for_item('<?php echo $serial_no; ?>');" class="form-control" required>
    				        ...
    				        <td>
                                ...
                                ?>
                                <option <?php if($item_s_no1[$i]==$item_s_no){ echo 'selected'; } ?> value="<?php echo $item_s_no; ?>" <?php if($available_stock<=0){ echo 'disabled'; } ?>><?php echo $item_name.' ['.$item_class.']-['.$item_description.']'; ?></option>
                                <?php } ?>
                                </select>
