<?php include("../attachment/session.php");
	...
	$item_class = $_POST['student_class'];
...
$indx=$index[$i];
$ledger_item_s_no=$ledger_item_s_no.$comma_var.$item_name[$indx];
$ledger_item_class=$ledger_item_class.$comma_var.$item_class;
$ledger_item_category=$ledger_item_category.$comma_var.$item_category[$indx];
$comma_var=',';

$quer="insert into sale_detail(sales_type,student_customer_id,invoice_number,invoice_date,item_class,item_category,item_s_no,quantity,rate,total_amount,total_quantity,grand_total,discount_remark,discount_amount,payable_amount,payment_mode,cheque_bank_name,cheque_date,cheque_number,neft_bank_name,neft_bank_acount_number,sale_status,session_value,$update_by_insert_sql_column) values('$sales_type','$student_customer_id','$invoice_no','$invoice_date','$item_class','$item_category[$indx]','$item_name[$indx]','$item_quantity[$indx]','$item_rate[$indx]','$item_total[$indx]','$grand_total_quantity','$item_grand_total','$total_discount_remark','$total_discount_amount','$total_payable_amount','$payment_mode','$cheque_bank_name','$cheque_date','$cheque_no','$neft_bank_name','$neft_bank_account_no','Active','$session1',$update_by_insert_sql_value)";
if(mysqli_query($conn73,$quer)){
    
    $query01="select available_stock from new_stock where item_s_no='$item_name[$indx]' and new_stock_status='Active'";
    ...
    $new_available_stock=$available_stock-$item_quantity[$indx];
    $query1="update new_stock set available_stock='$new_available_stock',$update_by_update_sql where item_s_no='$item_name[$indx]' and new_stock_status='Active'";
