...
$.ajax({
type: "POST",
url: access_link+"certificate/tc_delete_cbse.php?id="+s_no+"&roll_no="+roll_no+"",
cache: false,
...
while($row321=mysqli_fetch_assoc($run321)){
	$tc_cbse = $row321['tc_cbse'];
}	
?>
				<?php			
				$qry="select * from student_tc_cbse where student_tc_status='Active' and session_value='$session1' order by s_no DESC";
                ...
                $qry1="select student_status from student_admission_info where student_roll_no='$tc_student_roll_no' and student_status='Active' and session_value='$session1'";
              ...
              <td><a href='<?php echo $pdf_path; ?>certificate_page/<?php echo $tc_cbse; ?>?id=<?php echo $s_no; ?>' target="_blank"><button type="button" class="btn btn-success">Print</button></a></td>
			  <td><button type="button" onclick="post_content('certificate/tc_form_cbse_edit','<?php echo 'id='.$s_no; ?>')"  class="btn btn-success"><?php echo $language['Edit']; ?></button></td>
