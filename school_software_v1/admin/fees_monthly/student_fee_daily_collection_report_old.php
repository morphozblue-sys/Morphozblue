...
$.ajax({
	  type: "POST",
	  url: access_link+"fees_monthly/ajax_student_fee_daily_collection_report_old.php?class_name="+class_name+"&Payment_mode="+Payment_mode+"&section_name="+section_name+"&from_date="+from_date+"&to_date="+to_date+"&order_by="+order_by+"&transport_checked="+transport_checked+"&user_detail="+user_detail+"&fee_status="+fee_status+"",
	  cache: false,
	  success: function(detail){
...
<!------------------------------------------------Start Registration form--------------------------------------------------->
			...
			<div class="col-md-12" id="search_detail">
				...
				<option value=" ORDER BY editable_receipt_no ASC">By Receipt No.</option>
				<?php }else{ ?>
				...
				?>
				<option value="<?php echo $user_id.'|?|'.$user_email; ?>"><?php echo $user_name.' ['.$user_id.']'; ?></option>
				<?php } ?>
				</select>
				</div>
				</div>
