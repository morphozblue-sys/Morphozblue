				  ...
				  <table id="example1" class="table table-bordered table-striped" border="1px" cellpadding="10px" cellspacing="0" width="100%">
						<thead class="my_background_color">
								<tr>
								  ...
								  <th>Class (Sec)</th>
								  <th>Contact No</th>
								  <?php for($i=0;$i<$fees_count;$i++){ $grand_monthly_discount[$i]=0; ?>
					...
					for($m=0;$m<$serial_no;$m++){
						...
						$monthly_discount[$j]=$monthly_discount[$j]+($student_fee_amount-$student_fee_amount_after_disc);
						$total_discount_amount=$total_discount_amount+($student_fee_amount-$student_fee_amount_after_disc);
						$grand_monthly_discount[$j]=$grand_monthly_discount[$j]+($student_fee_amount-$student_fee_amount_after_disc);
					}
					}
					...
					<?php for($k=0;$k<$fees_count;$k++){ ?>
					<td><?php echo $monthly_discount[$k]; ?></td>
					<?php } ?>
					...
					<?php for($l=0;$l<$fees_count;$l++){ ?>
					<td><span style="font-weight:bold;"><?php echo $grand_monthly_discount[$l]; ?></span></td>
					<?php } ?>
