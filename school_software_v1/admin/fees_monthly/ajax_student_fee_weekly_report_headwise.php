...
?>
        ...
        <div class="col-md-12">
          ...
          <div class="box">
			...
			if($student_fee_head!='All'){
			    $student_fee_head1=explode('|?|',$student_fee_head);
			    $student_fee_head_code=$student_fee_head1[0];
			    $student_fee_head_name=$student_fee_head1[1];
			    $head_condition=" and fee_code='$student_fee_head_code'";
			}else{
			    $head_condition="";
			}
			...
			<td style="float:right"><b>Class (Sec) : <?php echo $class_name.' ( '.$section_name.' )'; ?></b></td>
				  ...
				  <?php
					$que0="select * from school_info_fee_types where fee_code!='' and session_value='$session1'$head_condition$filter37";
					...
					$fees_type_name = array('01'=>'Jan','02'=>'Feb','03'=>'Mar','04'=>'Apr','05'=>'May','06'=>'Jun','07'=>'Jul','08'=>'Aug','09'=>'Sep','10'=>'Oct','11'=>'Nov','12'=>'Dec');
				  ?>
				  ...
				  <table id="example1" class="table table-bordered table-striped" border="1px" cellpadding="10px" cellspacing="0" width="100%">
					...
					<tbody>
					<?php					
					$que="select * from student_admission_info where student_status='Active' and session_value='$session1'$condition1$condition2$condition3$order_by";
					...
					for($i1=0;$i1<$serial_no;$i1++){
					$student_headwise_amount[$i1]=0;
					}
					
					$que1="select * from common_fees_student_fee_add where fee_status='Active' and session_value='$session1' and student_roll_no='$student_roll_no'$frm_condition$to_condition";
					...
					$all_fee_rec_no=$all_fee_rec_no.$seprator.$reciept_no;
					$seprator=', ';
					$fee_paid_months=$row1['fee_paid_months'];
					$student_transport_fee_paid_total=$student_transport_fee_paid_total+$row1['student_transport_fee_paid_total'];
					$penalty_amount=$penalty_amount+$row1['penalty_amount'];
					$other_fee_amount=$other_fee_amount+$row1['other_fee_amount'];
					//$=$row1[''];
					...
					$headwise_total_amt[$m]=$headwise_total_amt[$m]+$row1[$fee_paid[$m].$fees_code[$j]];
					}
