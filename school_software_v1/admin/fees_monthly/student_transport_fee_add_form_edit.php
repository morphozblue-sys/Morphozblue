...
$.ajax({
	url: access_link+"fees_monthly/student_transport_fee_add_form_edit_api.php",
	type: "POST",
	data: formdata,
      ...
      <div class="row">
		...
		<div class="box-body  col-md-12">
			<div class="col-md-4">				
					...
					<?php
					$qry="select * from student_admission_info where student_status='Active' and registration_final='yes' and student_bus='Yes' and student_bus_fee_category_code!='' and session_value='$session1'";
	...
	?>
		...
		<center><h4 style="color:red;">Pay Transport Fees</h4></center>
            <?php
                ...
                $que11="select * from common_fees_student_transport_fee_add where s_no='$s_no11' and student_roll_no='$student_roll_no11' and session_value='$session1' and fee_status='Active'";
				...
				for($ag=0;$ag<$month_count11;$ag++){
				    //for($ai=0;$ai<$serial_no;$ai++){
				        $fee_paid11[$ag] = $row11['transport_fee_paid_month'.$month_exp[$ag]];
				    //}
				}
				...
				$que="select * from common_fees_student_transport_fee where student_roll_no='$student_roll_no' and session_value='$session1' and fee_status='Active'";
                $run=mysqli_query($conn73,$que);
		   ...
		   echo "<script>get_content('fees_monthly/set_classwise_transport_fee_details');</script>";
	       //echo "<script>window.open('set_fee_details.php?student_roll_no=$student_roll_no','_self')</script>";
				...
				<div class="col-md-6">
				<div class="col-md-12">
				<h4 style="color:green;"><?php for($h=0;$h<$fees_count;$h++){ if($month_exp[$g]==$fees_code[$h]){ echo $fees_type_name[$h]; } } ?> Transport Fee Set</h4>
				</div>
				<?php
				$show_total_fee=0;
				$show_paid_fee=0;
				...
				$fee1 = $row['transport_fee_month'.$month_exp[$g]];
				$fee_balance1 = $row['transport_fee_balance_month'.$month_exp[$g]]+$fee_paid11[$g];
				if($fee_balance1==''){
				$fee_balance1=0;
				}
				$fee_paid1 = $row['transport_fee_paid_month'.$month_exp[$g]]-$fee_paid11[$g];

				$total_amount_after_discount1 = $row['transport_fee_amount_after_discount_month'.$month_exp[$g]];
				if($total_amount_after_discount1==''){
				$total_amount_after_discount1=0;
				}
				$show_total_fee=$show_total_fee+$total_amount_after_discount1;
				$show_paid_fee=$show_paid_fee+$fee_paid1;
				$show_balance_fee=$show_balance_fee+$fee_balance1;
				?>
				...
				<div class="form-group">
                  ...
                  <input type="text" name="<?php echo 'transport_fee_amount_after_discount_month'.$month_exp[$g];?>" placeholder="Transport Fee" value="<?php echo $total_amount_after_discount1; ?>" id="" class="form-control fee" oninput="for_total();" readonly />
                </div>
				</div>
				
				<div class="col-md-4">				
				<div class="form-group">
                  <label><small>Transport Fee Balance</small></label>
                  <input type="text" name="<?php echo 'transport_fee_balance_month'.$month_exp[$g];?>" placeholder="0" value="<?php echo $fee_balance1; ?>" id="" class="form-control" readonly />
                </div>
				</div>
				
				<div class="col-md-4">				
				<div class="form-group">
                  <label><small>Transport Fee Paid</small></label>
                  <input type="number" name="<?php echo 'transport_fee_paid_month'.$month_exp[$g];?>" value="<?php echo $fee_balance1; ?>" placeholder="amount" id="" oninput="total_fee();" step="1" max="<?php echo $fee_balance1; ?>" class="form-control amt" />
				  
				  <input type="hidden" name="<?php echo 'fee_paid1_'.$g;?>" value="<?php echo $fee_paid1; ?>" />
                </div>
				</div>
				</div>
		    ...
		    <div class="col-md-12">
				...
				<div class="form-group">
				  ...
				  <!--<?php if($blank_field_5==''){ echo $blank_field_5+1;}else{echo $blank_field_5;} ?>-->
                  <input type="number" name="fee_receipt_no" placeholder="" value="" class="form-control"  >
                </div>
                </div>
				<div class="col-md-2">
				<div class="form-group">
                  <label><?php echo $language['Fee Submission Date']; ?></label>
                  <input type="date"  name="fee_submission_date" placeholder=""  value="<?php echo $fee_submission_date11; ?>" class="form-control" <?php if($_SESSION['sub_panel_submission_date_change']!='yes'){ echo "readonly"; } ?> >
                </div>
                </div>
				...
				<div class="form-group">
                  <label><?php echo $language['Payment Mode']; ?></label>
                    <select name="student_payment_mode" class="form-control" onchange="payment_mode(this.value);" required >
					  <option <?php if($student_payment_mode11==''){ echo 'selected'; } ?> value=""><?php echo $language['Select']; ?></option>
			          <option <?php if($student_payment_mode11=='Cash'){ echo 'selected'; } ?> value="Cash"><?php echo $language['Cash']; ?></option>
					  <option <?php if($student_payment_mode11=='Cheque'){ echo 'selected'; } ?> value="Cheque"><?php echo $language['Cheque']; ?></option>
			          <option <?php if($student_payment_mode11=='NEFT'){ echo 'selected'; } ?> value="NEFT"><?php echo $language['NEFT Net Banking']; ?></option>
