...
var receipt_status=document.getElementById('receipt_status').value;
if($('#student_bus_fee_category').prop("checked") == true){
var student_bus_fee_category=$('#student_bus_fee_category').val();
}else{
var student_bus_fee_category="";
}

if($('#student_bus_route').prop("checked") == true){
var student_bus_route=$('#student_bus_route').val();
}else{
var student_bus_route="";
}
...
$.ajax({
	  type: "POST",
	  url: access_link+"fees_monthly/ajax_student_fee_daily_collection_report_new.php?class_name="+class_name+"&section_name="+section_name+"&from_date="+from_date+"&to_date="+to_date+"&order_by="+order_by+"&student_bus_fee_category="+student_bus_fee_category+"&student_bus_route="+student_bus_route+"&student_bus="+student_bus+"&user_detail="+user_detail+"&fee_status="+fee_status+"&receipt_status="+receipt_status+"",
	  cache: false,
	  success: function(detail){
      ...
      <div class="row">
          ...
          <div class="box box-primary my_border_top">
            <div class="box-header with-border">
              ...
              <div class="col-md-2">
              <input type="checkbox" id="student_bus_route" onclick="for_list();" value="student_bus_route" /> <b>Print Bus route</b>
              </div>
			  ...
			  <select class="form-control" name="receipt_status" id="receipt_status" onchange="for_list();">
			  <option value="Active">Active</option>
			  <option value="All">All</option>
...
<!------------------------------------------------Start Registration form--------------------------------------------------->
			...
			<div class="col-md-12" id="search_detail">
				...
				<div class="form-group">
				<label>Bus</label>
				<select class="form-control" name="student_bus" id="student_bus" onchange="for_list();">
				<option value="All">All</option>
				...
				<option value=" ORDER BY <?php echo 'CAST('.$var.' AS UNSIGNED)'; ?> ASC">By Receipt No.</option>
				...
				<?php 
				$sql11= "select * From user_rights";
				$result11=mysqli_query($conn73,$sql11) or die(mysqli_error($conn73));
				while($row11=mysqli_fetch_assoc($result11)){
                $user_name=$row11['user_name'];
                $user_email=$row11['user_email'];
				?>
				<option value="<?php echo $user_email; ?>"><?php echo $user_name.' ['.$user_email.']'; ?></option>
				<?php } ?>
				</select>
				</div>
				</div>
