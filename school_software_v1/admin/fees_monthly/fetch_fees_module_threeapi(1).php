...
$sql2.=" GROUP BY common_fees_student_fee_add.student_roll_no ORDER BY MAX(common_fees_student_fee_add.fee_submission_date) DESC";
...
if(mysqli_num_rows($query4)>0){
    ...
    $sql3 ="select fee_submission_date,$fee_rec_column_name,blank_field_2 from common_fees_student_fee_add where student_roll_no='$student_roll_no_temp' and fee_status='Active' and session_value='$session1'";
    ...
    while($row3=mysqli_fetch_assoc($query3)){
        ...
        $total_discount+=$row3['blank_field_2'];
        $comma=',';
    }
    ...
    $subdata[]=$row['student_class'];
	 ...
	 $subdata[]=$fee_submission_date1;
		...
		$student_roll_no=$row['student_roll_no'];
                    ...
                    $subdata[]="$total_discount";
                    $subdata[]="".$total_discount+$row['paid_total'];
					$subdata[]=$row['balance_total'];
		
 $subdata[]="<button type='button' onclick="."post_content('fees_monthly/student_fee_list_particular','student_roll_no=".$student_roll_no."');"." class='btn btn-default my_background_color'>Details</button>";
	$data[]=$subdata;
	$serial_no++;
