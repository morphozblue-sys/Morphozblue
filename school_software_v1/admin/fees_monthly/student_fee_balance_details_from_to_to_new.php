...
function for_feelist(){
   ...
   var with_previous=document.getElementById('with_previous').value;
   
    if($('#with_previous').prop("checked") == true){
    var previous=$('#with_previous').val();
    }else{
    var previous="";
    }
   ...
   $.ajax({
	  type: "POST",
	  url: access_link+"fees_monthly/ajax_student_fee_balance_details_monthwise_new.php",
	  data: {student_class:student_class,student_section:student_section,month_code:month_code,month_name:month_name,student_fee_category:student_fee_category,order_by:order_by,previous:previous},
	  cache: false,
	  success: function(detail){
    ...
    <section class="content">
      <div class="row">
		...
		<div class="col-md-2">
			<input type="checkbox" id="with_previous" value="previous" onclick="for_feelist();" /><span style="font-weight:bold;color:red;"> With Previous </span>
		</div>
