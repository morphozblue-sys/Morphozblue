...
$blank_field_column_name="blank_field_5";
	...
	$other_fee_amount = intval($_POST['other_fee_amount']);
	...
	$student_previous_year_fee_paid = intval($_POST['student_previous_year_fee_paid']);
	$student_previous_year_fee_paid_total = intval($_POST['student_previous_year_fee_paid_total']);
	...
	$total_paid = intval($_POST['total_paid']);
	$paid_total = intval($_POST['paid_total']);
	$paid_total = $paid_total+$total_paid;
	$balance_total = intval($_POST['balance_total']);
 ...
 $que = "select fcm_token from fcm_info where roll_number='$student_roll_no'";
	...
	$query="insert into ledger_info (emp_id_or_student_roll_no,emp_or_student_name,date,amount_type,payment_mode,total_amount,credit_or_debit_from,session_value,office_account_sno,$update_by_insert_sql_column) values('$student_roll_no','$student_name','$fee_submission_date','Credit','$student_payment_mode','$total_paid','fee','$session1','$office_account_sno',$update_by_insert_sql_value)";
	...
	//	$principal_sms="Dear Sir, Your Student ".$student_name." ".$student_class." - ".$student_class_section." Paid ".$total_paid." Rs.";
		$principal_sms="Dear Sir, Your Student ".$student_name."(".$student_class." - ".$student_class_section.") Paid Rs. ".$total_paid.". [SCHOOL]";

		//sendDNDSMS($school_info_school_contact_no,$sms);
		}
		$medium=''; $shift=''; $board='';
