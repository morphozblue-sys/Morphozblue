...
function add_edit(code, name) {
  ...
  $('#si_feec_modal_title').text(parts[0] ? 'Edit Fee Category' : 'Add Fee Category');
}

$(document).ready(function () {
  $("#my_form").off('submit').on('submit', function (e) {
    e.preventDefault();
    var fc = $.trim($('#fee_type1').val());
    if (fc === '') { alert_new('Fee category name is required.', 'red'); return; }
    ...
    $.ajax({
      ...
      success: function (detail) {
        ...
        if (res[1] === 'success') {
          alert_new('Fee category saved successfully!', 'green');
...
<div style="padding:0 10px 30px;">
  ...
  <div class="si-card">
    <div class="si-card-header">
      <div class="si-hdr-ico"><i class="fa fa-list-alt"></i></div>
      <h4>Fee Category Management</h4>
    </div>
    <div class="si-card-body">

      <?php
      $que = "select * from school_info_fee_category";
      $run = mysqli_query($conn73, $que) or die(mysqli_error($conn73));
      $serial_no = 0; $add_more_button = 0; $fee_code_blank = ''; $last_name = ''; $last_hindi = '';
      $rows = [];
      while ($row = mysqli_fetch_assoc($run)) {
        if ($row['category_name'] !== '' || $row['category_name_hindi'] !== '') {
          $rows[] = $row; $serial_no++;
        } else {
          if ($add_more_button === 0) $fee_code_blank = $row['category_code'];
          $last_name  = $row['category_name'];
          $last_hindi = $row['category_name_hindi'];
          $add_more_button++;
        }
      }
      ?>
        ...
        <table class="si-tbl">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><?php echo htmlspecialchars($r['category_name_hindi']); ?></td>
              <td class="cen">
                <button type="button"
                  class="si-btn si-btn-ok"
                  style="padding:5px 14px;font-size:12px;"
                  id="<?php echo htmlspecialchars($r['category_code']); ?>"
                  name="<?php echo htmlspecialchars($r['category_name'].'|?|'.$r['category_name_hindi']); ?>"
            ...
            <tr>
              <td colspan="4" class="cen" style="padding:12px;">
                <button type="button"
                  ...
                  name="<?php echo htmlspecialchars($last_name.'|?|'.$last_hindi); ?>"
                  onclick="add_edit(this.id,this.name)"
                  data-toggle="modal" data-target="#myModal">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-header">
          ...
          <h4 class="modal-title" id="si_feec_modal_title">Add / Edit Fee Category</h4>
        </div>

        <div class="modal-body">
          <div class="si-fg">
            <label>Fee Category Name <span class="req">*</span></label>
            <div class="si-iw">
          ...
          <div class="si-fg" style="margin-bottom:0;">
            <label>Fee Category Hindi</label>
            <div class="si-iw">
