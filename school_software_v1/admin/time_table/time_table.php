...
if (!mysqli_query($conn73, $query)) {
    $alter = "ALTER TABLE `school_info_class_period` ADD `class_code` VARCHAR(20) NOT NULL AFTER `s_no`, ADD `period_code` VARCHAR(20) NOT NULL AFTER `class_code`;";
    mysqli_query($conn73, $alter);
}
...
while ($row = mysqli_fetch_assoc($run)) {
    ...
    if ($period_code == '') {
        mysqli_query($conn73, "update school_info_class_period set period_code='period1' where s_no='1'");
        mysqli_query($conn73, "update school_info_class_period set period_code='period2' where s_no='2'");
        mysqli_query($conn73, "update school_info_class_period set period_code='period3' where s_no='3'");
        mysqli_query($conn73, "update school_info_class_period set period_code='period4' where s_no='4'");
        mysqli_query($conn73, "update school_info_class_period set period_code='period5' where s_no='6'");
        mysqli_query($conn73, "update school_info_class_period set period_code='period6' where s_no='7'");
        mysqli_query($conn73, "update school_info_class_period set period_code='period7' where s_no='8'");
        mysqli_query($conn73, "update school_info_class_period set period_code='period8' where s_no='9'");
        mysqli_query($conn73, "update school_info_class_period set period_code='period9' where s_no='5'");
        break;
    }
}
?>
...
<section class="content-header">
  <h1><?php echo $language['Time Table Management']; ?> <small><?php echo $language['Control Panel']; ?></small></h1>
  <ol class="breadcrumb">
    <li><a href="javascript:get_content('index_content')"><i class="fa fa-dashboard"></i> <?php echo $language['Home']; ?></a></li>
    <li><a href="javascript:get_content('time_table/time_table')"><i class="fa fa-calendar"></i> <?php echo $language['Time Table']; ?></a></li>
...
<?php
function tt_card($link, $grad, $glow, $icon, $title, $sub = 'Open') {
...
function tt_section($icon, $label, $badge_grad, $count = 0) {
...
?>
  ...
  <div class="dash-section">
    <?php tt_section('fa-clock-o', 'Same Period Timing — All Classes', 'linear-gradient(135deg,#06b6d4,#0284c7)', 7); ?>
    <div class="dash-grid">

      <?php if (!isset($_SESSION['time_table_sub_panel_class_periode'])) : ?>
        <?php tt_card('time_table/add_class_period',
          'linear-gradient(135deg,#06b6d4,#0284c7)', 'rgba(6,182,212,.45)',
          'fa-clock-o', 'Class Period', 'Set Periods'); ?>
        <?php if ($_SESSION['database_name'] == 'simptkfv_jamiafatimaniswa') : ?>
          <?php tt_card('time_table/add_class_period_new',
            'linear-gradient(135deg,#0891b2,#0e7490)', 'rgba(8,145,178,.45)',
            'fa-clock-o', 'Class Period New', 'Set Periods'); ?>
        <?php endif; ?>
      <?php endif; ?>

      <?php if (!isset($_SESSION['time_table_sub_panel_subjectwise_teacher'])) : ?>
        <?php tt_card('time_table/subject_wise_teacher',
          'linear-gradient(135deg,#9333ea,#7e22ce)', 'rgba(147,51,234,.45)',
          'fa-user-circle', 'Subject Wise Teacher', 'Assign Teachers'); ?>
      <?php endif; ?>

      <?php if (!isset($_SESSION['sub_panel_time_table_generate'])) : ?>
        <?php tt_card('time_table/time_table_generate',
          'linear-gradient(135deg,#e8414e,#c0102d)', 'rgba(232,65,78,.45)',
          'fa-table', $language['Time Table'], 'Generate'); ?>
        <?php if ($_SESSION['database_name'] == 'simptkfv_jamiafatimaniswa') : ?>
          <?php tt_card('time_table/time_table_generate_new',
            'linear-gradient(135deg,#dc2626,#991b1b)', 'rgba(220,38,38,.45)',
            'fa-table', 'Time Table New', 'Generate'); ?>
        <?php endif; ?>
      <?php endif; ?>

      <?php if (!isset($_SESSION['sub_panel_time_table_list'])) : ?>
        <?php tt_card('time_table/time_table_list',
          'linear-gradient(135deg,#9F2B68,#7B1D4F)', 'rgba(159,43,104,.45)',
          'fa-list-alt', $language['Time Table List'], 'View List'); ?>
        <?php if ($_SESSION['database_name'] == 'simptkfv_jamiafatimaniswa') : ?>
          <?php tt_card('time_table/time_table_list_new',
            'linear-gradient(135deg,#be185d,#9d174d)', 'rgba(190,24,93,.45)',
            'fa-list-alt', 'Time Table List New', 'View List'); ?>
          <?php tt_card('time_table/time_table_list_new_teacherwise',
            'linear-gradient(135deg,#db2777,#be185d)', 'rgba(219,39,119,.45)',
            'fa-list-alt', 'Time Table List New', 'Teacherwise'); ?>
        <?php endif; ?>
      <?php endif; ?>

      <?php if (!isset($_SESSION['sub_panel_teacher_availability'])) : ?>
        <?php tt_card('time_table/teacher_availability',
          'linear-gradient(135deg,#2ecc71,#1a9e56)', 'rgba(46,204,113,.45)',
          'fa-calendar-check-o', $language['Teacher Availability'], 'Check Availability'); ?>
      <?php endif; ?>

      <?php if (!isset($_SESSION['time_table_sub_panel_teacher_mgt'])) : ?>
        <?php tt_card('time_table/teacher_assign',
          'linear-gradient(135deg,#3B6AA7,#2a4f7e)', 'rgba(59,106,167,.45)',
          'fa-users', 'Teacher Management', 'Manage Teachers'); ?>
      <?php endif; ?>

      <?php if (!isset($_SESSION['time_table_sub_panel_timetable_sheet'])) : ?>
        <?php tt_card('time_table/time_table_sheet',
          'linear-gradient(135deg,#65a30d,#3f6212)', 'rgba(101,163,13,.45)',
          'fa-file-text', 'Time Table Sheet', 'View Sheet'); ?>
      <?php endif; ?>

    </div>
  </div>

  <!-- ═══ DIFFERENT PERIOD TIMING ═══ -->
  <div class="dash-section">
    <?php tt_section('fa-clock-o', 'Different Period Timing — Per Class', 'linear-gradient(135deg,#d97706,#92400e)', 3); ?>
    <div class="dash-grid">

      <?php if (!isset($_SESSION['time_table_sub_panel_class_periode1'])) : ?>
        <?php tt_card('time_table/diff_add_class_period',
          'linear-gradient(135deg,#06b6d4,#0284c7)', 'rgba(6,182,212,.45)',
          'fa-clock-o', 'Class Period', 'Set Periods'); ?>
      <?php endif; ?>

      <?php if (!isset($_SESSION['time_table_sub_panel_time_table1'])) : ?>
        <?php tt_card('time_table/diff_time_table_generate',
          'linear-gradient(135deg,#e8414e,#c0102d)', 'rgba(232,65,78,.45)',
          'fa-table', $language['Time Table'], 'Generate'); ?>
      <?php endif; ?>

      <?php if (!isset($_SESSION['time_table_sub_panel_time_table_list1'])) : ?>
        <?php tt_card('time_table/diff_time_table_list',
          'linear-gradient(135deg,#9333ea,#7e22ce)', 'rgba(147,51,234,.45)',
          'fa-list-alt', $language['Time Table List'], 'View List'); ?>
      <?php endif; ?>
