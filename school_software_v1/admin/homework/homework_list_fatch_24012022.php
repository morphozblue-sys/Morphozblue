...
echo $sql1 ="select * from homework_student where session_value='$session1'$condition$condition1$condition2$condition3 ORDER BY s_no DESC";
...
while($row=mysqli_fetch_assoc($query)){
    ...
    $homework_name=$row['blank_field_1'];
 	...
 	$homework_file=htmlspecialchars($row['homework_file_name']);
 $new_image=$_SESSION['amazon_file_path']."homework_document/".$homework_file;
    ...
    $subdata[]="<button type='button' onclick=post_content('smartclass/homework_edit','id=$s_no') class='btn btn-info' >Edit</button>";    
