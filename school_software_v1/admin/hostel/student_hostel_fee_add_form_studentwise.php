...
$.ajax({
	  type: "POST",
	  url: access_link+"hostel/ajax_class_section_all.php?class_name="+value+"",
	  cache: false,
	  success: function(detail){
...
$.ajax({
	  type: "POST",
	  url: "ajax_get_student_for_hostel_challan_list.php?student_class="+student_class+"&student_section="+student_section+"&category_code="+category_code+"&student_roll_no="+student_roll_no+"",
	  cache: false,
	  success: function(detail){
          ...
          <div class="box <?php echo $box_head_color; ?>" >
            ...
            <div class="box-body table-responsive">
			  ...
			  <div class="panel-body">
				...
				?>
				<option <?php if(isset($_GET['student_roll_no'])){ if($_GET['class_code']==$class_code){ echo 'selected'; } } ?> value="<?php echo $class_name; ?>"><?php echo $class_name; ?></option>
				<?php } ?>
				</select>
				</div>
				<div class="col-sm-2">
				<label>Section</label>
				<select name="" id="student_section_search" class="form-control" onchange="for_students();">
				<option value="All">All</option>
				...
				<select name="" id="student_category_search" class="form-control" onchange="for_students();">
				<option value="All">All</option>
				<option <?php if(isset($_GET['student_roll_no'])){ if($_GET['category_code']=='category3'){ echo 'selected'; } } ?> value="category3">New Hostlers</option>
				<option <?php if(isset($_GET['student_roll_no'])){ if($_GET['category_code']=='category4'){ echo 'selected'; } } ?> value="category4">Old Hostlers</option>
				</select>
				</div>
				<div class="col-sm-2"></div>
				...
				<select name="student_roll_no" id="student_name" style="width:100%;" class="form-control select2" onchange="for_class(this.value);" required >
				...
				<select name="installment_number" id="installment_number" style="width:100%;" class="form-control " onchange="get_insatllment_amount();" required >
				<option value="">Select Installment</option>
				<option <?php if(isset($_GET['student_roll_no'])){ if($_GET['installment_number']=='installment1'){ echo 'selected'; } } ?> value="installment1">installment1</option>
				<option <?php if(isset($_GET['student_roll_no'])){ if($_GET['installment_number']=='installment2'){ echo 'selected'; } } ?> value="installment2">installment2</option>
				<option <?php if(isset($_GET['student_roll_no'])){ if($_GET['installment_number']=='installment3'){ echo 'selected'; } } ?> value="installment3">installment3</option>
				<option <?php if(isset($_GET['student_roll_no'])){ if($_GET['installment_number']=='installment4'){ echo 'selected'; } } ?> value="installment4">installment4</option>
				</select>
				</div>
			  </div>
...
<?php if($installment_no1=='installment1'){ ?>
		...
		<div class="col-md-6">
		<h4><b><input type="checkbox" name="extra_challan" id=""> Remaining Amount Challan</b></h4>
		</div>
		</div>
 ...
 <?php } ?>
				...
				?>
				<td><input type="text" name="" id="<?php echo 'my_total_'.$fee_head_code[$w]; ?>" class="total_payable" value="<?php echo $after_discount_amount[$w]-$fees_paid_amount[$w]; ?>" style="color:#FF0000;width:60px;" readonly /></td>
				<td><input type="text" name="student_hostel_balance[]" id="" class="total_balance" value="" style="color:#FF0000;width:60px;" readonly /></td>
				<td><input type="text" name="student_hostel_fee[]" id="<?php echo 'my_pay_'.$fee_head_code[$w]; ?>" class="total_pay" value="<?php echo $after_discount_amount[$w]-$fees_paid_amount[$w]; ?>" style="color:#0000FF;width:60px;" readonly /></td>
				</tr>
				<?php
				}
				?>
				...
				<td id="penalty" >
				<?php
				$que23="select * from student_hostel_fees_paid where student_roll_no='$student_roll_no23' and session_value='$session1'";
				...
				$que22="select * from hostel_due_date_schedule where student_due_installment='$installment_no23' and student_due_class_code='$class_code' and session_value='$session1'";
				...
				?>
				<input type="text" name="penalty_pay" id="penalty_pay" class="" value="<?php echo $penalty_amount22; ?>" style="color:#0000FF;width:60px;" oninput="installment_calculation();" />
				</td>
				</tr>
				<tr>
...
$query021="select * from student_hostel_fees_paid where student_roll_no='$student_roll_no' and installment_no='$installment_number' and session_value='$session1' or student_roll_no='$student_roll_no' and all_fee='Yes' and session_value='$session1'";
$result021=mysqli_query($conn73,$query021);

$query21="select * from student_hostel_fees_paid where student_roll_no='$student_roll_no' and installment_no='$installment_number' and session_value='$session1' or student_roll_no='$student_roll_no' and all_fee='Yes' and session_value='$session1'";
$result21=mysqli_query($conn73,$query21);

if(isset($_POST['extra_challan'])){
$check_condition=(mysqli_num_rows($result021)>0);
$msg_var="extra";
}else{
$check_condition=(mysqli_num_rows($result21)<1);
$msg_var="installment";
}
...
$hostel_paid_column_value=$hostel_paid_column_value.",'$student_hostel_fee[$ad]'";
$hostel_paid_column_value1=$hostel_paid_column_value1.",'$student_hostel_balance[$ad]'";
}
$query22="insert into student_hostel_fees_paid(student_roll_no,total_amount,installment_no,challan_no,payment_full_partial,partial_remark,ready_for_verify,penalty_amount,session_value,all_fee$hostel_paid_column_name,$update_by_insert_sql_column) values('$student_roll_no','$total_pay_final2','$installment_number','$hostel_challan_no','$payment_full_partial','$current_challan_remark','Yes','$penalty_pay','$session1','$all_fee'$hostel_paid_column_value,$update_by_insert_sql_value)";
...
echo "<script>window.open('student_hostel_fee_add_form_studentwise.php?student_roll_no=$student_roll_no&class_code=$student_class_code&category_code=$category_code&installment_number=$installment_number&student_class=$student_class&student_category=$student_category','_self');</script>";
}
}else{
if($msg_var=='extra'){
echo "<script>alert_new('This Challan is Not Valid !!!');</script>";
}elseif($msg_var=='installment'){
echo "<script>alert_new('This Record is Already Exists !!!');</script>";
}
}
}
?>
