...
<div class="wrapper"> <?php include("../attachment/header.php"); ?>  <?php include("../attachment/sidebar.php"); ?>
  ...
  $query5="select * from expense_monthly where s_no='$s_no' and session_value='$session1'";
  $result5=mysqli_query($conn73,$query5)or die(mysqli_error($conn73));
  $arr_sno=0;
  while($row5=mysqli_fetch_assoc($result5)){
  $student_roll_no111=$row5['student_roll_no'];
  $create_date=$row5['create_date'];
  $create_time=$row5['create_time'];
  $from_date=$row5['from_date'];
  $to_date=$row5['to_date'];
  $total_amount=$row5['total_amount'];
  for($k=0;$k<$sl_no;$k++){
  $fee_head_amount[$arr_sno]=$row5[$fee_head_code[$k]];
  $fee_head_remark[$arr_sno]=$row5[$fee_head_code[$k].'_remark'];
  $arr_sno++;
  }
  }
  if($create_date!=''){
  $create_date1=explode('-',$create_date);
  $create_date=$create_date1[2].'-'.$create_date1[1].'-'.$create_date1[0];
  }
  ?>  
      ...
      <div class="row">
          ...
          <div class="box box-primary my_border_top">
            <div class="box-header with-border ">
              <h3 class="box-title">Student Action</h3><span style="float:right;"><b><?php echo $create_date; ?></b></span>
            </div>
            <!-- /.box-header -->
			...
			<div class="col-md-6 ">				
					<div class="form-group" >
					  ...
					  <option value="">Select Student</option>
							...
							?>
							<option <?php if($student_roll_no111==$student_roll_no){ echo 'selected'; } ?> value="<?php echo $student_roll_no; ?>"><?php echo $student_name."[".$student_admission_number."]-"."[".$student_class."-".$student_class_section."]-[".$student_father_name."-".$student_father_contact_number."]"; ?></option>
							<?php
							}
							?>
					  </select>
...
<!---table----------------------------------------------------------------------->				
		<div class="box-body table-responsive" id="showing_box">
            ...
            <tr>
			    ...
			    <td><input type="text" name="fee_monthly_amount[]" class="form-control amt" id="" value="<?php echo $fee_head_amount[$i]; ?>" style="width:80px;" oninput="calculate();" /></td>
				<td><textarea rows="2" cols="50" name="fee_monthly_remarks[]"><?php echo $fee_head_remark[$i]; ?></textarea></td>
		    </tr>	
			<?php } ?>
		    <tr>
                <td><label>Total Amount</label></td>
			    <td><label id="for_total"><?php echo $total_amount; ?></label></td>
				<td>&nbsp;</td>
		    </tr>
           <tr>
...
<?php
	...
	$fee_column=$fee_column.",$fee_code[$i]='$fee_monthly_amount[$i]'";
	$fee_column1=$fee_column1.",$fee_code[$i]_remark='$fee_monthly_remarks[$i]'";
	}
	
	$query="update expense_monthly set student_roll_no='$student_roll_no',from_date='$from_date',to_date='$to_date',total_amount='$total_amount'$fee_column$fee_column1,$update_by_update_sql  where s_no='$s_no' and session_value='$session1'";
