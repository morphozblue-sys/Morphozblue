...
var tc_issued=document.getElementById('tc_issued').value;
...
$.ajax({
	  type: "POST",
	  url: access_link+"account/ajax_ledger_report.php?from_date="+from_date+"&to_date="+to_date+"&tc_issued="+tc_issued+"&user_name="+user_name+"",
	  cache: false,
...
<!------------------------------------------------Start Registration form--------------------------------------------------->
			...
			<div class="col-md-12 col-md-offset-3" id="search_detail">
				...
				<input type="date" name="to_date" class="form-control" id="to_date" value="<?php echo date('Y-m-d'); ?>" oninput="for_list();" />
				 ...
				 <div class="col-md-2">				
					<div class="form-group">
					  <label>TC Issued</label>
					    <select id="tc_issued" class="form-control" onchange="for_list()" >
				 ...
				 <div class="col-md-2">				
					<div class="form-group">
					  <label>User Name</label>
					    <select id="user_name" class="form-control" onchange="for_list()" >
					    <option value="All">All</option>
					    <?php 
					    $query1="select * from user_rights where user_name!='simption'";
					    $run1=mysqli_query($conn73,$query1) or die(mysqli_error($conn73));
                        while($row=mysqli_fetch_assoc($run1)){
                        ?>
                            <option value="<?php echo $row['user_email']; ?>"><?php echo "$row[user_name]"; ?></option>
                        <?php 
                        }
					    ?>
					    </select>
